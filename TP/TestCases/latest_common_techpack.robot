...
${path}    /root/CI/cicd/bin/activate/workspace/TP_ROBOT_Test
...
Common Topology
    #Test
    #checking services
    Verify the latest dc TP in Adminui
...
Verify the latest dc TP in Adminui
    Set Client Configuration    prompt=eniqs[eniq_stats] {dcuser} #:    timeout=3600
    Open clearcasevobs
    ${rstate}    Get Text    //table//a[text()='${dc_pkg}']/../following-sibling::td[3]
    ${temp}=    Get Element Attribute    //table//a[text()='${dc_pkg}']    href
    ${temp1}=    Fetch From Right    ${temp}    /
    Set Global Variable    ${full_pkg_name}    ${temp1}
    Write    echo -e "select TOP 1 TECHPACK_VERSION from Versioning where TECHPACK_NAME = '${dc_pkg}' ORDER BY TECHPACK_VERSION Desc\\n${run}\\n" | isql -P ${password_db} -U ${username} -S ${database} -b
    ${output1}=    Read Until Prompt
    ${rstate_adminui}=    Filter Name    ${output1}
    IF    '${rstate}' != '${rstate_adminui}'
        Go To    ${solarisLink}
        ${currentpkg}=    Get Element Attribute     xpath=//a[text()='${dc_pkg}']    href
...
Installing latest TP
    IF    '${latest_TP_found}' == 'True'
        ...
        Write    cd /eniq/sw/installer ; ./tp_installer -p . -t ${dc_pkg}
        #Read Until Prompt
        ${out}=    Read Until Prompt
    END
verifying TP Installation
    IF    '${latest_TP_found}' == 'True'
        Set Client Configuration    prompt=eniqs[eniq_stats] {dcuser} #:    timeout=3600
        Write    echo -e "select TECHPACK_VERSION from Versioning where TECHPACK_NAME='${dc_pkg}'\\n${run}\\n" | isql -P ${password_db} -U ${username} -S ${database} -b
        ${output1}=    Read Until Prompt
        ${new_rstate_inDB}=    Filter Latest Name    ${output1}
        Write    echo -e "select VERSIONID from Versioning where TECHPACK_NAME='${dc_pkg}'\\n${run}\\n" | isql -P ${password_db} -U ${username} -S ${database} -b
        ${output2}=    Read Until Prompt 
        ${temp}=    Filter Latest Name    ${output2}
        ${temp1}=    Give Build Number    ${temp}
        ${full_pkg_name_in_db}=    Catenate    SEPARATOR=_    ${dc_pkg}    ${new_rstate_inDB}    b${temp1}.tpi    
...
Verify TP installer Log
    IF    '${latest_TP_found}' == 'True'
        ${stat}=    Checking Tp Installer Log    ${dc_pkg}    ${full_pkg_name_in_db}    ${host}    ${port}    ${uname}    ${pwd}
        IF    '${stat}' == 'fail'
            Fail
        END
    END
...
Fetch Rstate of Interface from AdminUI and Clearcase
    Set Client Configuration    prompt=eniqs[eniq_stats] {dcuser} #:    timeout=3600
    Write    echo -e "select INTERFACENAME from InterfaceTechpacks where TECHPACKNAME='${dc_pkg}'\\n${run}\\n" | isql -P ${password_db} -U ${username} -S ${database} -b
    ${output1}=    Read Until Prompt
...
Activating Interface
    Set Client Configuration    prompt=eniqs[eniq_stats] {dcuser} #:    timeout=3600
    Write    cd /eniq/sw/installer ; ./activate_interface -o eniq_oss_1 -t ${dc_pkg}
    ${out}=     Read Until Prompt
...
Setting Loader Configuration To Finest
    ...
    ${output1}=    Read Until Prompt
    ${dep_int}=    Filter Interfaces    ${output1}
    Filter Intf Name    ${dc_pkg}    ${output1}    ${host}    ${uname}    ${pwd}
...
Generating Topology files
    ...
    IF    '${dir_exist}' == 'true'
        ...
        ${out}=    Read Until Prompt
    END
    Write     echo -e "${dc_pkg}_R10A_b12.tpi" | perl epfgtopofiles.pl 
    ${out1}=    Read Until Prompt
...
Loading Topology Files
    ...
    END
Loading Topo checking
    ${res}=    Loading Topo Check    ${dc_pkg}    ${host}    ${port}    ${uname}    ${pwd}
    IF    '${res}' == 'false'
        Fail
    END
Verify All Loader Tables
...
Area Files check for DIM_RAN_AREA
    ...
    IF    '${dc_pkg}' == 'DIM_E_UTRAN'
        ...
        ${out}=    Read Until Prompt
        ${res}=    Verify Area File Data Loading    ${dc_pkg}    ${out}
        IF    '${res}'=='false'
            Fail
        END
    END
...
Area Files check for LTE and NR
    ...
    IF    '${dc_pkg}' == 'DIM_E_LTE'
        ...
        ${res}=    Verify Area File For Lte Nr    ${dc_pkg}    ${out1}    ${out2}
        IF    '${res}'=='false'
            Fail
        END
    END
