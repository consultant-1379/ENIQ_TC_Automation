...
###############################################################
	...
	for ($i=0;$i<$count;$i++) {
		...
		my %PmGenNodes = (
				...
				"vPP" => ["vPPGenFlag"],
				"ADC" => ["cIMSGenFlag"],
				"NRFAGENT" => ["CCDMGenFlag" , "CCSMGenFlag" , "CCRCGenFlag" , "CCESGenFlag" , "CCPCGenFlag"],
			);
		...
		if ( $d =~ m/(^DC_\w*)/ ) {	
			...
			if ($string ~~ @keys){
				...
				for ($j=0;$j<$count1;$j++){
					...
					print "\nPattern in epfg is $pattern\n";
					
					my $oldstartnode = $flagName."StartNode=";
					print "\nStratNode in epfg is $oldstartnode\n";
					my $newstartnode = $flagName."StartNode=5\n";

					my $oldendnode = $flagName."EndNode=";
					print "\nEndNode in epfg is $oldendnode\n";
					my $newendnode = $flagName."EndNode=5\n";

					my $utctimezone = $flagName."UtcTimeZone=";
					my $newutctimezone = $flagName."UtcTimeZone=+0200\n";

					my $suspectflag = $flagName."SuspectFlag=";
					my $newsuspectflag = $flagName."SuspectFlag=YES\n";

					my $suspectsfvalue = $flagName."SuspectSfValue=";
                    my $newsuspectsfvalue = $flagName."SuspectSfValue=true\n";
					...
					my $newEndTime = $flagName."EndTime=".$yesterdayDate."-10:15\n";
					...
					foreach(@epfg_lines) {
						...
						if($_=~ m/($oldstartnode)/){
							$_ =~ s/$_/$newstartnode/g;
						}
						if($_=~ m/($oldendnode)/){
							$_ =~ s/$_/$newendnode/g;
						}
						if($_=~ m/($utctimezone)/){
							$_ =~ s/$_/$newutctimezone/g;
						}
						if($_=~ m/($suspectflag)/){
							$_ =~ s/$_/$newsuspectflag/g;
						}
						if($_=~ m/($suspectsfvalue)/){
                            $_ =~ s/$_/$newsuspectsfvalue/g;
                        }
