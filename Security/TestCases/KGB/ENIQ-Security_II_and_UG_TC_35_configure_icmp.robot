...
Test firewall status
    ${result}=      Execute Command       systemctl status firewalld | grep -i Active | cut -d':' -f 2 | cut -d ' ' -f 2
    ${result1}=      Execute Command       systemctl status firewalld | sed -n '/Loaded:/p' | cut -d ';' -f 2 | cut -d ' ' -f 2
...
Test ICMP block check
    ${result1}=      Execute Command       firewall-cmd --query-icmp-block=redirect
    ${result2}=      Execute Command       firewall-cmd --query-icmp-block=timestamp-reply
    ${result3}=      Execute Command       firewall-cmd --query-icmp-block=router-solicitation
    ${result4}=      Execute Command       firewall-cmd --query-icmp-block=router-advertisement
...
Test ICMP ECHO not ignored
    ${result}=  Execute Command     cat /etc/sysctl.conf | grep net.ipv4.icmp_echo_ignore_all
...
Test ICMP ECHO not ignored is configured in sysctl daemon
    ${result}=  Execute Command     sysctl -n net.ipv6.conf.all.accept_ra
