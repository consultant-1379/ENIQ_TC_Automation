...
Test disable icmp responses is not set to 1 - Negative scenario
   ...
   Execute Command           sed -i 's/net.ipv4.icmp_ignore_bogus_error_responses=1/net.ipv4.icmp_ignore_bogus_error_responses=0/g' /root/sysctl_backup.conf
   Log To Console            \nNegative Scenario contents are written to backup file\n
   ${negative_expected}       Execute Command         cat /root/sysctl_backup.conf | grep  net.ipv4.icmp_ignore_bogus_error_responses
...
Test disable icmp responses is set to 1 - Positive scenario
    ${result}=      Execute Command       cat /etc/sysctl.conf | grep net.ipv4.icmp_ignore_bogus_error_responses
...
Test disabling icmp responses is enforced in sysctl daemon - verifying system daemon
    ${sysctl_check1}=        Execute Command       sysctl -n net.ipv4.icmp_ignore_bogus_error_responses
