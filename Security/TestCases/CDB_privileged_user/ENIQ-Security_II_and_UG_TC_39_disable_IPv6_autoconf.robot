...
Test disable Ipv6 autoconf parameter are not set to 0 - Negative scenario
   ...
   Execute Command           sudo sed -i 's/net.ipv6.conf.default.autoconf=0/net.ipv6.conf.default.autoconf=1/g' /root/sysctl_backup.conf
   Log To Console            \nNegative Scenario contents are written to backup file\n
   ${negative_expected}       Execute Command         sudo cat /root/sysctl_backup.conf | grep  net.ipv6.conf.default.autoconf
...
Test disable Ipv6 autoconf parameter is set to 0 - positive scenario
    ${result}=      Execute Command       sudo cat /etc/sysctl.conf | grep net.ipv6.conf.default.autoconf
...
Test disabling Ipv6 autoconf parameter is enforced in sysctl daemon - Verifying system daemon
    ${sysctl_check1}=        Execute Command       sudo sysctl -n net.ipv6.conf.default.autoconf
