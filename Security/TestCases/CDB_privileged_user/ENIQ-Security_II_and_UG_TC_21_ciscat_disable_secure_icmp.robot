...
Test disable secure icmp parameters values are not 0 - Negative scenario
   ...
   Execute Command           sudo sed -i 's/net.ipv4.conf.all.secure_redirects=0/net.ipv4.conf.all.secure_redirects=1/g; s/net.ipv4.conf.default.secure_redirects=0/net.ipv4.conf.default.secure_redirects=1/g' /root/sysctl_backup.conf
   Log To Console            \nNegative Scenario contents are written to backup file\n
   ${negative_expected}       Execute Command         sudo grep  -e "net.ipv4.conf.all.secure_redirects" -e "net.ipv4.conf.default.secure_redirects" /root/sysctl_backup.conf 
...
Test disable secure icmp parameters values are 0 - Positive scenario
    ${result}=      Execute Command       sudo cat /etc/sysctl.conf | grep net.ipv4.conf.all.secure_redirects
    ...
    ${result1}=      Execute Command       sudo cat /etc/sysctl.conf | grep net.ipv4.conf.default.secure_redirects
...
Test disabling secure icmp parameters is enforced in sysctl daemon - verifying system daemon
    ...
    ${sysctl_check1}=        Execute Command       sudo sysctl -n net.ipv4.conf.all.secure_redirects
    Should Be Equal    ${sysctl_check1}    0
    ${sysctl_check2}=        Execute Command       sudo sysctl -n net.ipv4.conf.default.secure_redirects
