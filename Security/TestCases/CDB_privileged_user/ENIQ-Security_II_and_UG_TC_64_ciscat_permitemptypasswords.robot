...
Test PermitEmptyPasswords parameter is not set to no - Negative scenario
   ...
   Execute Command           sudo sed -i 's/PermitEmptyPasswords no/PermitEmptyPasswords yes/g' /root/sshd_backup.conf
   Log To Console            \nNegative Scenario contents are written to backup file\n
   ${negative_expected}       Execute Command         sudo cat /root/sshd_backup.conf | grep  PermitEmptyPasswords
...
Test PermitEmptyPasswords parameter is no - Positive scenario
    ${result}=      Execute Command     sudo cat /etc/ssh/sshd_config | grep PermitEmptyPasswords
...
Test PermitEmptyPasswords is enforced on SSH daemon - verifying system kernel
    ${result}=        Execute Command       sudo sshd -T | grep permitempty
