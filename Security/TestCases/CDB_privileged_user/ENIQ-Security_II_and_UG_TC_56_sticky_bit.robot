...
Test sticky bit not set for /etc    
    ${result}=               Execute Command              sudo chmod -t /etc
    ${cmd}=                  Execute Command              sudo ls -ld /etc | head -n 1 | cut -d'.' -f1
    ${get_lines_mat}=        Get Lines Matching Regexp    ${cmd}=       drwxr-xr-x
    Should Be Equal          ${cmd}                       drwxr-xr-x
    ${result_1}=             Execute Command              sudo chmod +t /etc

Test sticky bit of folder /etc    
    ${result}=               Execute Command              sudo ls -ld /etc | head -n 1 | cut -d'.' -f1
