...
Test GSSAPIAuthentication parameter is not set to no - Negative scenario
   ...
   Execute Command           sed -i 's/GSSAPIAuthentication no/GSSAPIAuthentication yes/g' /root/sshd_backup.conf
...
Test SSAPIAuthentication parameter is set to no - Positive scenario
    ${result}=  Execute Command   cat /etc/ssh/sshd_config | grep GSSAPIAuthentication
...
Test SSAPIAuthentication parameter is set to no in SSH daemon - Verifying system daemon
    ${result}=               Execute Command              sshd -T | grep gssapiauthentication
