...
Test disable icmp broadcast parameter values are not 1 - Negative scenario
   ...
   Execute Command           sed -i 's/net.ipv4.icmp_echo_ignore_broadcasts=1/net.ipv4.icmp_echo_ignore_broadcasts=0/g' /root/sysctl_backup.conf
   Log To Console            \nNegative Scenario contents are written to backup file\n
   ${negative_expected}       Execute Command         cat /root/sysctl_backup.conf | grep  net.ipv4.icmp_echo_ignore_broadcasts
...
Test disable icmp broadcast parameter values are 1 - Positive scenario
    ${result}=               Execute Command              cat /etc/sysctl.conf | grep net.ipv4.icmp_echo_ignore_broadcasts
...
Test ignore icmp broadcast is enforced in sysctl daemon - verifying system daemon
    ${sysctl_check1}=        Execute Command       sysctl -n net.ipv4.icmp_echo_ignore_broadcasts
...
Test disable ip forward parameter values are not 0 - Negative scenario
   ...
   Execute Command           cp /etc/sysctl.conf /root/sysctl_backup.conf
   Log To Console            \nSuccessfully backed up config file.\n
   Execute Command           sed -i 's/net.ipv4.ip_forward=0/net.ipv4.ip_forward=1/g' /root/sysctl_backup.conf
   Log To Console            \nNegative Scenario contents are written to backup file\n
   ${negative_expected}       Execute Command         cat /root/sysctl_backup.conf | grep  net.ipv4.ip_forward
...
Test disable ip forward parameter values are 0 - Positive scenario
    ${result}=               Execute Command              cat /etc/sysctl.conf | grep net.ipv4.ip_forward
...
Test ip forward is enforced in sysctl daemon - verifying system daemon
    ${sysctl_check1}=        Execute Command       sysctl -n net.ipv4.ip_forward
