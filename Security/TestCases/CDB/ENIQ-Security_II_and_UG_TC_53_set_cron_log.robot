...
Test wheather syslog is set or not
    ${maillog}=  Execute Command     cat /etc/logrotate.d/syslog | grep /var/log/maillog
    ${messages}=  Execute Command     cat /etc/logrotate.d/syslog | grep /var/log/messages
    ${secure}=  Execute Command     cat /etc/logrotate.d/syslog | grep /var/log/secure
    ${spooler}=  Execute Command     cat /etc/logrotate.d/syslog | grep /var/log/spooler
    #${syslogd_pid}=  Execute Command     cat /etc/logrotate.d/syslog | grep /bin/kill -HUP `cat /var/run/syslogd.pid 2> /dev/null` 2> /dev/null || true
...
Test wheather logrotate.d/cron is set or not
    ...
    ${daily}=    Execute Command     cat /etc/logrotate.d/cron | grep daily
    ${create}=  Execute Command     cat /etc/logrotate.d/cron | grep create
    ${dateext}=  Execute Command     cat /etc/logrotate.d/cron | grep dateext
