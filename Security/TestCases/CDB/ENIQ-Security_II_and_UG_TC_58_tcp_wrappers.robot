...
Test vsftpd not present in hosts deny file
    ${cmd}=                  Execute Command       sed -i 's/ALL/NO/g' /etc/hosts.deny
    ${result}=               Execute Command   cat /etc/hosts.deny | grep vsftpd
    ${get_lines_mat}=        Get Lines Matching Regexp    ${result}      vsftpd: NO
    Should Be Equal          ${result}       vsftpd: NO
    ${cmd_1}=                Execute Command       sed -i 's/NO/ALL/g' /etc/hosts.deny

Test hosts allow file is present
    ${result}=               Execute Command   find /etc/hosts.allow
...
Test hosts deny file is present
    ${result}=               Execute Command   find /etc/hosts.deny
