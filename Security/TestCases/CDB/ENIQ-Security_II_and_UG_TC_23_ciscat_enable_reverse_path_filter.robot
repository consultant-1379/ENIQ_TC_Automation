...
Test enable reverse path_filter parameters values are not 1 - Negative scenario
   ...
   Execute Command           sed -i 's/net.ipv4.conf.all.rp_filter=1/net.ipv4.conf.all.rp_filter=0/g; s/ net.ipv4.conf.default.rp_filter=1/net.ipv4.conf.default.rp_filter=0/g' /root/sysctl_backup.conf
   Log To Console            \nNegative Scenario contents are written to backup file\n
   ${negative_expected}       Execute Command         grep  -e "net.ipv4.conf.all.rp_filter" -e "net.ipv4.conf.default.rp_filter" /root/sysctl_backup.conf 
...
Test enable reverse path_filter parameters values are 1 - Positive scenario
    ${result}=      Execute Command       cat /etc/sysctl.conf | grep net.ipv4.conf.all.rp_filter
    ...
    ${result1}=      Execute Command       cat /etc/sysctl.conf | grep net.ipv4.conf.default.rp_filter
...
Test reverse path filter is enforced in sysctl daemon - verifying system daemon
    ${sysctl_check1}=        Execute Command       sysctl -n net.ipv4.conf.all.rp_filter
    Should Be Equal    ${sysctl_check1}    1
    ${sysctl_check2}=        Execute Command       sysctl -n net.ipv4.conf.default.rp_filter
