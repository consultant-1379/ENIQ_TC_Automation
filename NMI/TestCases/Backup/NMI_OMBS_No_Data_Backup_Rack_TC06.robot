...
Checking if get_ombs_info.bsh script execution successfully started
    [Tags]                 OMBS     B&R
    ${OMBS}=               Get File            OMBS-No-Data-Backup-Rack.log
...
Checking if User selected option is NoData
    [Tags]             OMBS  B&R  
    Should Contain     ${OMBS}                 User selected option is NoData
...
Checking if crontab successfully updated
    [Tags]             OMBS  B&R  
    Should Contain     ${OMBS}                 Successfully added /eniq/bkup_sw/bin/raw_backup_supervisor.bsh in crontab
...
Installing NBU client software in ENIQ Backup Policy process
    [Tags]             OMBS  B&R    
    Should Contain     ${OMBS}                 Installing NBU client software
