...
Checking if database connection is made for dcuser
	...
	${user}=                    Execute Command             cat /eniq/installation/config/niq.ini | grep ETLREPUsername= | cut -f1 -d'=' --complement| head -1
	${strong_passphase}=        Execute Command             cat /eniq/sw/conf/strong_passphrase
	...
	${encryption_flag}=         Execute Command         	cat "/tmp/dcuser_password" | cut -d "'" -f2 
	${output1}= 				Execute Command         	cat "/tmp/dcuser_password" | cut -d "'" -f4
	
	IF							'${encryption_flag}' == 'Y'
	${db_dcuser_pwd}			Execute Command             echo '${output1}' | openssl base64 -d
	ELSE IF						'${encryption_flag}' == 'YY'
	${db_dcuser_pwd}			Execute Command             echo '${output1}' | openssl enc -aes-256-ctr -md sha512 -a -d -salt -pass pass:${value1}
	END

	${output2}=                 Execute Command         	cat /eniq/sw/conf/dcuser_password_file
	${dcuser_pwd}				Execute Command             echo '${output2}' | openssl enc -aes-256-ctr -md sha512 -a -d -salt -pass pass:${value1}
	
	Should Contain				${db_dcuser_pwd}			${dcuser_pwd}
	
	Write						rm -rf /tmp/dcuser_password
	Write 						cat /tmp/dcuser_password
	${output}=					Read 						delay=3s
...
Checking the flag of password
    ...
    ${value}=                   Execute Command             cat /eniq/installation/config/niq.ini | grep -i ${user_type}Password= | cut -f1 -d'=' --complement| head -1
    ${state}=                   Execute Command             cat /eniq/installation/config/niq.ini | grep -i ${user_type}Password_Encrypted= | cut -f1 -d'=' --complement| head -1
    ${value1}=                  Execute Command             cat /eniq/sw/conf/strong_passphrase
	...
	Write                       su - dcuser -c 'dbisql -nogui -c "eng=repdb;links=tcpip{host=repdb;port=2641};uid=${user_type};pwd=${pwd}"'
	...
	Write                       su - dcuser -c 'dbisql -nogui -c "eng=dwhdb;links=tcpip{host=dwhdb;port=2640};uid=${user_type};pwd=${pwd}"'
