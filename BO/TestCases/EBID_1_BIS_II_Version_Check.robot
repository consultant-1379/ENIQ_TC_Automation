...
*** Test Cases ***
Automated BI server status from automation summary log
    ${result}=    Run Ps    server    ((Get-Content -Path ${BO_Log_path}\\ebid_automation_summary.log | Select-string -pattern "Automated BusinessObjects Install"))
...
BIS version check from registry
