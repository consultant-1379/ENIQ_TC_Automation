...
{ 
	...
	{		
		...
		$global:InstallConfigFile43Sp3P3 = "C:\ebid\install\ebid_bi43_sp3_p3_server.ini"
		
		if(Test-Path $InstallConfigFile43Sp3P3)
		{
			$global:InstallConfigFile = $InstallConfigFile43Sp3P3
		}
	
		elseif(Test-Path $InstallConfigFile43Sp3)
		{
...
Function CheckHealthStatus()
{
	...
	try
	{
		...
		{			
			...
			if([bool](Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* |Where-Object {$_.DisplayName -Match 'BI platform 4.3 SP3'}))
		            {
			            "`n Installed BIS version is SP3" >>$global:LogFile
		                 CheckBIServerStatusNowSP3
		
		            }
            else
                    {
	                     "`nBIS Versions installed lower than 4.3 SP3" >>$global:LogFile
...
{                        
    ...
    (Get-Content $global:NewInstallini) -replace $regex4, "productkey=$global:ProductKey" | Set-Content $global:NewInstallini
...
# ---------------------------------------------------------------------------------------------
#  Function to Find the Health Status of the server when upgrade is from 4.3 SP3 TO 4.3 SP3 P3
# ----------------------------------------------------------------------------------------------


Function CheckBIServerStatusNowSP3()
{
	PrintDateTime
	"`nChecking the Health Status of Bi Server Now" >>$global:LogFile
	try
	{		
		...
		while((($global:StoppedServer -ne 0) -OR ($global:DisabledServer -ne 0) -OR ($global:RunningServer -ne $global:enabledServer)) -AND ($global:WaitingTime -le 40))
		{
			...
			foreach ($line in $obj1)
			{
				if (Select-String -InputObject $line -Pattern "State:" -AllMatches) 
				{
                   $Server_State=$line.trim()
		           if($Server_State -eq "Running with Errors State:")
		            {
		        	   $Server_State = "Running with Errors"
		            }
		           else{
		              ...
		              if($Server_State -eq "Running")
		               {
			                  $running_server = $running_server + 1
			                   # $global:Running_Server = $global:Running_Server + 1 
		                 }
		               elseif($Server_State -eq "Failed")
		                 {
			                 $failed_server = $failedserver + 1
			  
		                   }
		              elseif($Server_State -eq "Starting")
		             {
			                $starting_server = $starting_server + 1
		                 }
		               elseif($Server_State -eq "Stopped")
		              {
			                 $stopped_server = $stopped_server + 1 
		                  }
		        }
			...
			{
				$enabled_server = $enabled_server + 1
			}
			elseif( $Server_Enabled_Disabled -eq "Disabled")
			{
				$disabled_server = $disabled_server + 1
			}
	  }
			}
		   
			
			$global:RunningServer = $running_server
	
			
			$global:FailedServer = $failed_server
	
			
			$global:StartServer = $starting_server
	
			
			$global:StoppedServer = $stopped_server
	
			
			$global:EnabledServer = $enabled_server
	
			
			$global:DisabledServer = $disabled_server
	...
	{
		$_ >>$global:LogFile
		"`nException in CheckBIServerStatusNowSP3 function" >>$global:LogFile
		$time = get-date -Format "yyyy MM dd HH:mm:ss"
		write-host "`n$time : [ERROR] Check $global:LogFile for more details." -ForegroundColor Red
		Exit
	}	
...
# ------------------------------------------------------------------------
# Function: Product Key prompt during BIS Upgrade
# ------------------------------------------------------------------------
function ProductKeyRequired()
{
	PrintDateTime
	...
	$global:license_registry = $false
	$global:license_inventory = $false

	"Checking if 'product_key_required' parameter is required to be Updated.." >> $global:LogFile
	
	# ------------------------------------------------------------------------
	# Product Key: Registry Checks for BI Platform 4.3
	# ------------------------------------------------------------------------
	$BISVersionName = "BI platform 4.3"
	"Product/License Key update check using Registry is In-Progress.." >> $global:LogFile

	"Initial Value of 'license_registry': $global:license_registry" >> $global:LogFile
	
	if ([bool](Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -Match $BISVersionName}))
	{
		"'$BISVersionName' is found in Registry checks" >> $global:LogFile
		$global:license_registry = $false
	}
	else
	{
		"'$BISVersionName' is not found in Registry checks" >> $global:LogFile
		$global:license_registry = $true
	}
	
	"Final Value of 'license_registry': $global:license_registry" >> $global:LogFile

	# ------------------------------------------------------------------------
	# Product Key: Inventory Checks for BI Platform 4.3
	# ------------------------------------------------------------------------
	$bi_inventory = $bi_install_dir + "InstallData\inventory.txt"
	"Product/License Key update check using inventory.txt is In-Progress.." >> $global:LogFile

	"Initial Value of 'license_inventory': $global:license_inventory" >> $global:LogFile

	if (Test-Path -Path $bi_inventory)
	{
		"'$bi_inventory' is found" >> $global:LogFile
		$bi_ver = Get-Content $bi_inventory
		foreach ($line in $bi_ver)
		{
			if ($line -match "BI platform 4.3")
			{
				"'$BISVersionName' is found in inventory.txt checks" >> $global:LogFile
				$global:license_inventory = $false
			...
			{
				"'$BISVersionName' is not found in inventory.txt checks, checking again.." >> $global:LogFile
				$global:license_inventory = $true
			}
		}
	}
	else
	{
		"'$bi_inventory' is not found" >> $global:LogFile
		$global:license_inventory = $false
	}

	"Final Value of 'license_inventory': $global:license_inventory" >> $global:LogFile
	
	# ------------------------------------------------------------------------
	# Product Key: Required OR Not Required
	# ------------------------------------------------------------------------
    if (($global:license_registry -eq $true) -OR ($global:license_inventory -eq $true))
	{
		"Product/License Key is required" >> $global:LogFile
        ...
        "Updated 'product_key_required = Yes' in 'ebid_automation.ini' file" >> $global:LogFile

		$global:ProductKey = Read-Host "`nEnter Product Key"

        # ------------------------------------------------------------------------
        # Product Key: EBID License File Updates
        # ------------------------------------------------------------------------
        PrintDateTime
        "Updates in 'ebid_license.ini' file is required" >> $global:LogFile
        
        if (Test-Path -Path "$global:LicenseUpdatePath\ebid_license.ini")
        {
            "'$global:LicenseUpdatePath\ebid_license.ini' is already present" >> $global:LogFile
        }
        else
        {
            "'$global:LicenseUpdatePath' is not present" >> $global:LogFile

            "Copying 'update_license' directoty to 'C:\ebid\' directory" >> $global:LogFile
            Copy_Folder -source_Directory $global:UpdateLicenseFileToCopy -destination_Directory "C:\ebid\"
            "'update_license' directoty is successfully copied to 'C:\ebid\' directory" >> $global:LogFile
        }
		...
		"Updates in 'ebid_license.ini' file is Completed" >> $global:LogFile
	}
	else
	{
		"Product/License Key is not required" >> $global:LogFile
		
		(Get-Content $global:MasterScriptConfigFile) -replace '^product_key_required =.+$', "product_key_required = No" | Set-Content $global:MasterScriptConfigFile

        "Updated 'product_key_required = No' in 'ebid_automation.ini' file" >> $global:LogFile
	}
}
...
#Check if BI 4.3 SP3 Patch 3 is installed in Server
