...
Verify login failure details in adminuistatus.log when adminui user password is expired
    ...
    IF    not ${length} == 0
        FOR    ${user}    IN    @{expired_users}
            ...
            Page Should Contain Element    //*[contains(text(),'password has expired')]
            Close Browser
            ${adminuistatus_log}    ${rc}    Execute Command    cat /eniq/log/sw_log/adminui/adminuiStatus.log | tail -1 | grep -i "Login failed , password has expired."    return_rc=True
            IF    ${rc} == '1'
                Fail    Failing the Test-case, since issue with log file.${\n}Below is the output-${\n}${adminuistatus_log}
            ELSE
                Log    Log file verified, no issues occured.${\n}Passing the test-case.
            END
        END
    ELSE
        Log    No user's password is expired. Passing the test-case
    END
