        ...
        IF    ${check_platform_folder} == True
            ...
            IF    ${latest_sprint_present} == True
                ...
                IF    ${check_module_on_nexus} == True
                    ...
                    END
                ELSE
                    ${get_pf_features}=    Execute Command    ls /net/10.45.192.134/JUMP/ENIQ_STATS/ENIQ_STATS/ | grep -ix Features_*.*_${Shipment}
                    Should Not Be Empty    ${get_pf_features}
                    ${check_mws_module}=    Execute Command    ls /net/10.45.192.134/JUMP/ENIQ_STATS/ENIQ_STATS/${get_pf_features}/eniq_parsers/ | grep -i ^${pf_mod_rstate_out}
    ...
    FOR    ${element}    IN    @{pf_parsers_module_list} 
        ...
        IF    ${check_platform_folder} == True
            ...
            IF    ${latest_sprint_present} == True
                ${check_module_on_nexus}=    Run Keyword And Return Status     Should Contain Match   ${module_list}     regexp=^${element}_
                IF    ${check_module_on_nexus} == True
                    ${mws_modules_status}=    Run Keyword And Return Status    Should Contain    ${latest_module}    ${vdb_mod_rstate_out}
                    IF    ${mws_modules_status} == False
                ...
                ELSE
                     ${get_pf_features}=    Execute Command    ls /net/10.45.192.134/JUMP/ENIQ_STATS/ENIQ_STATS/ | grep -ix Features_*.*_${Shipment}
                    Should Not Be Empty    ${get_pf_features}
                    ${check_mws_module}=    Execute Command    ls /net/10.45.192.134/JUMP/ENIQ_STATS/ENIQ_STATS/${get_pf_features}/eniq_parsers/ | grep -i ^${vdb_mod_rstate_out}
                    ${mws_modules_status}=    Run Keyword And Return Status    Should Not Be Empty    ${check_mws_module}
                    IF    ${mws_modules_status} == False
                        Append To List    ${modules_mismatch_list}    ${vdb_mod_rstate_out}
                    END
