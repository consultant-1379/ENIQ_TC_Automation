...
Verify Reduced delay Interface Alarm report addition
    ...
    Verify target reports are not added    ${Reduced_delay_cdb_basetable}    ${Reduced_delay_cdb_alm_template}    Reduced Delay
    Click Add report button
    Select alarm template from available reports    ${Reduced_delay_cdb_alm_template}
    Select basetable from dropdown list    ${Reduced_delay_cdb_basetable}
    Click Add alarm report button
    Verify alarm report addition     ${Reduced_delay_cdb_basetable}    ${Reduced_delay_cdb_alm_template}

Verify Reduced delay Interface Alarm report disabled
    Skip if    '${PREV TEST STATUS}'=='FAIL' or '${PREV TEST STATUS}'=='SKIP'
    Disable Alarm report    Reduced Delay    ${Reduced_delay_cdb_basetable}    ${Reduced_delay_cdb_alm_template}
...
Verify Reduced delay Interface Alarm report enabled
    ...
    Enable Alarm report    Reduced Delay    ${Reduced_delay_cdb_basetable}    ${Reduced_delay_cdb_alm_template}
    Verify message is displayed    successfully enabled

Verify Reduced delay Interface Alarm report deletion
    Skip if    '${PREV TEST STATUS}'=='SKIP'
    Delete Alarm report    Reduced Delay    ${Reduced_delay_cdb_basetable}    ${Reduced_delay_cdb_alm_template}
