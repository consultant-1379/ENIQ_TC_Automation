...
Verify 10min Interface Alarm report addition
    ...
    Verify target reports are not added    ${cdb_basetable}    ${cdb_alm_template}    10min
...
Verify 10min Interface Alarm report disabled
    Skip if    '${PREV TEST STATUS}'=='FAIL' or '${PREV TEST STATUS}'=='SKIP'
    Disable Alarm report    10min    ${cdb_basetable}    ${cdb_alm_template}
    Verify message is displayed    successfully disabled

Verify 10min Interface Alarm report enabled
    Skip if    '${PREV TEST STATUS}'=='SKIP'
    Enable Alarm report    10min    ${cdb_basetable}    ${cdb_alm_template}
    Verify message is displayed    successfully enabled

Verify 10min Interface Alarm report deletion
    Skip if    '${PREV TEST STATUS}'=='SKIP'
    Delete Alarm report    10min    ${cdb_basetable}    ${cdb_alm_template}
