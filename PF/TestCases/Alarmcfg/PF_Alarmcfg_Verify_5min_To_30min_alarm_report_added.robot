...
${5min_alm_template_db}    AM_Static_Threshold_Template_AUTOMATION-MTAS-1
${5min_basetable_db}    DC_E_MTAS_MTASLICENSES_RAW
...
${30min_alm_template_db}    AM_Static_Threshold_Template_AUTOMATION-MTAS-1
${30min_basetable_db}    DC_E_MTAS_MTASLICENSES_RAW
...
Verify 5min Interface Alarm report addition
    ...
    Select alarm template from available reports    ${5min_alm_template_db}
    Select basetable from dropdown list    ${5min_basetable_db}
    Click Add alarm report button
    Verify alarm report addition     ${5min_basetable_db}    ${5min_alm_template_db}
...
Verify 30min Interface Alarm report addition
    Launch the Alarmcfg webpage in browser
    Input Alarmcfg Login Details    ${almcfg_sys}    ${almcfg_user}    ${almcfg_pass}    ${almcfg_auth}
    Click Login Button
    ...
    Verify no reports are added
    Click Add report button
    Select alarm template from available reports    ${30min_alm_template_db}
    Select basetable from dropdown list    ${30min_basetable_db}
    Click Add alarm report button
    Verify alarm report addition     ${30min_basetable_db}    ${30min_alm_template_db}
    [Teardown]    Test teardown steps
...
Check alarmcfg reports added status
    Execute Command    date '+%Y-%m-%d %H:%M:%S' > /var/tmp/5min_almreport_db_add_time.txt
    Run Keyword If    '${TEST STATUS}'=='PASS'    Execute Command    echo 'True' > /var/tmp/5min_almreport_add_status.txt   ELSE    Execute Command    echo 'False' > /tmp/5min_almreport_add_status.txt  
...
Check alarmcfg reports added status
    Execute Command    date '+%Y-%m-%d %H:%M:%S' > /var/tmp/15min_almreport_db_add_time.txt
    Run Keyword If    '${TEST STATUS}'=='PASS'    Execute Command    echo 'True' > /var/tmp/15min_almreport_add_status.txt   ELSE    Execute Command    echo 'False' > /tmp/15min_almreport_add_status.txt  
...
Check alarmcfg reports added status
    Execute Command    date '+%Y-%m-%d %H:%M:%S' > /var/tmp/almreport_db_add_time.txt
    Run Keyword If    '${TEST STATUS}'=='PASS'    Execute Command    echo 'True' > /var/tmp/30min_almreport_add_status.txt   ELSE    Execute Command    echo 'False' > /tmp/30min_almreport_add_status.txt  
