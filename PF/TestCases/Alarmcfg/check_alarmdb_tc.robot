...
${5min_alm_template_db}    AUTOMATION_ERBSG2_EUTRANCELLTDD_15MIN
${5min_basetable_db}    DC_E_ERBSG2_EUTRANCELLTDD_RAW
${5min_almrpt_name}    AlarmInterface_5min
${reports_alarmname_db}    AUTOMATION_ERBSG2_EUTRANCELLTDD_15MIN

*** Test Cases ***
Verify 5min alarmreport status is active 
...
Functional verification 5min alarm type -report parsing verification in file logs
    ...
    Log To Console    ${check_data_parsing}
    Should Not Be Empty    ${check_data_parsing}

# Functional verification 5min alarm type -report parsing verification in engine logs
#     Skip if    '${PREV TEST STATUS}'=='FAIL' or '${PREV TEST STATUS}'=='SKIP'
#     # ${almcfg_current_date}=    Execute Command    date +"%Y_%m_%d"
#     ${almcfg_current_date}=    Set Variable    2023_12_20  
#     ${check_Alarminterface_engine_file}=    Execute Command    ls /eniq/log/sw_log/engine/AlarmInterfaces/ | grep -i engine-${almcfg_current_date}.log
#     Log To Console    ${check_Alarminterface_engine_file}
#     Should Not Be Empty    ${check_Alarminterface_engine_file}
#     # # ${check_data_parsing}=    Execute Command    awk -F' ' '/^[0-9]{2}\.[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}/ {if ($2 >= "${start_5min_time_parse_logs}") flag=1} flag' /eniq/log/sw_log/engine/AlarmInterfaces/file-${almcfg_current_date}.log | grep -i ${5min_almrpt_name} | grep -i ${5min_alm_template_db}| grep -i parsed | tail -10
#     ${check_data_parsing}=    Execute Command    cat /eniq/log/sw_log/engine/AlarmInterfaces/engine-${almcfg_current_date}.log | grep -i "${5min_almrpt_name}" | grep -i "Triggering set Loader_DC_Z_ALARM_INFO"
#     Log To Console    ${check_data_parsing}
#     Should Not Be Empty    ${check_data_parsing}

Functional verification 5min alarm type -report loading verification in DB
    Skip if    '${PREV TEST STATUS}'=='FAIL' or '${PREV TEST STATUS}'=='SKIP'
    ...
    ${almcfg_current_date}=    Set Variable    2023-12-20
    # Write   echo -e "Select ROWSTATUS from DC_Z_ALARM_INFO_RAW where ReportTitle='${almcfg_5min_almrpt_id}' and TIMELEVEL='5MIN' and DATETIME_ID>='${5min_almcfg_current_datetime_db}';\ngo\n" | dbisql -n -c "UID=dc;PWD=${dc_pwd};eng=dwhdb" -host localhost -port 2640 -nogui
    ...
    Log To Console    ${almrpt_loaded_list}
