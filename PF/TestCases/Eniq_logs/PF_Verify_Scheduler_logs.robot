...
Check Scheduler start log is present         
    ${check_startfile}=    Execute Command    ls -lrth /eniq/log/sw_log/scheduler | grep -i "\\bstart_scheduler_" | tail -1
    Log   ${check_startfile}
...
Check Scheduler start log for any exceptions         
    Skip If    ${check_startfile_status} == False    msg=Start scheduler file not present
    ${start_sch}=    Execute Command    ls -Art /eniq/log/sw_log/scheduler | grep -i "\\bstart_scheduler_" | tail -1
    Log    ${start_sch}
    ${file_status}=    Run Keyword And Return Status    Should Not Be Empty    ${start_sch}
    IF    ${file_status} == True
        ${check_file}=    Execute Command    cat /eniq/log/sw_log/scheduler/${start_sch} | grep -i "is running OK"
        Should Contain    ${check_file}    is running OK  
    ELSE
        Skip    Start scheduler file not present
    END

Check Scheduler stop log is present         
    ${check_stopfile}=    Execute Command    ls -lrth /eniq/log/sw_log/scheduler | grep -i "\\bstop_scheduler_" | tail -1
    Log   ${check_stopfile}
...
Check Scheduler stop log for any exceptions         
    Skip If    ${check_stopfile_status} == False    msg=Stop scheduler file not present
    ${stop}=    Execute Command    ls -Art /eniq/log/sw_log/scheduler | grep -i "\\bstop_scheduler_" | tail -1
    Log    ${stop}
...
Check Schedulerheap log is present        
    ...
    ${check_scheapfile}=    Execute Command    ls -lrth /eniq/log/sw_log/scheduler | grep -i "\\bschedulerHeap-${ct_scheap_date}"
...
Check Schedulerheap log for any exceptions        
    Skip If    ${check_scheapfile_status} == False    msg=schedulerHeap file not present
    ${ct_scheap_date}=    Execute Command    date +"%Y-%m-%d"
    ${scheap_file}=    Execute Command    ls -Art /eniq/log/sw_log/scheduler | grep -i "\\bschedulerHeap-${ct_scheap_date}"
    Log   ${scheap_file}
    ${file_status}=    Run Keyword And Return Status    Should Not Be Empty    ${scheap_file}
    Log    ${file_status}
    IF    ${file_status} == True
        ${check_scheap_logs}    ${stderr}=    Execute Command    awk -v date="$(date --date='30 minutes ago' +'%H:%M')" '$4 > date' /eniq/log/sw_log/scheduler/${scheap_file} | grep -i "\\bexception\\b\\|\\bsevere\\b\\|\\bfailed\\b\\|\\berror\\b"    return_stderr=True
...
Check Scheduler log is present        
    ...
    ${check_schedulerfile}=    Execute Command    ls -lrth /eniq/log/sw_log/scheduler | grep -i "\\bscheduler-${ct_scheduler_date}" | tail -1
...
Check Scheduler log for any exceptions        
    Skip If    ${check_schfile_status} == False    msg=scheduler log file not present
    ${ct_scheduler_date}=    Execute Command    date +"%Y_%m_%d"
    ${sch_file}=    Execute Command    ls -Art /eniq/log/sw_log/scheduler | grep -i "\\bscheduler-${ct_scheduler_date}"
    Log   ${sch_file}
    ${file_status}=    Run Keyword And Return Status    Should Not Be Empty    ${sch_file}
    Log    ${file_status}
    IF    ${file_status} == True
        ${check_sch_logs}    ${stderr}=    Execute Command    awk -v date="$(date --date='30 minutes ago' +'%H:%M')" '$2 > date' /eniq/log/sw_log/scheduler/${sch_file} | grep -i "\\bexception\\b\\|\\bsevere\\b\\|\\bfailed\\b\\|\\berror\\b"    return_stderr=True
        Should Be Empty    ${stderr}  
