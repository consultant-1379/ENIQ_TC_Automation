...
Get etrep creds from database
    ...
    ${cmd_output}=    Read Until Prompt    strip_prompt=True
    Write    /eniq/sw/installer/getPassword.bsh -u ETLREP | awk '{print $NF}'
    ${etlrep_pwd_spaces}=    Read Until Prompt    strip_prompt=True
    ${etlrep_pwd}=    Strip String     ${etlrep_pwd_spaces}
