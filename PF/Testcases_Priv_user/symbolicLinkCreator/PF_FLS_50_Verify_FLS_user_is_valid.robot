...
Verify the FLS user is valid
    ...
    FOR    ${element}    IN    @{server_details}
        ...
        ${activating}    Execute Command    sudo su - dcuser -c "chmod u+x /eniq/sw/platform/${repo_package}/bin/${script}"
        ${pwd}    Execute Command     sudo su - dcuser -c "/eniq/sw/platform/${repo_package}/bin/EncryptDecryptUtility.sh -d ${host_details}[4]"   
        ${Verify_msg}    Execute Command    sudo su - dcuser -c "curl -k -L -c cookie.txt -X POST 'https://${host_details}[1]/login' -d IDToken1=${host_details}[3] -d IDToken2=${pwd}"
