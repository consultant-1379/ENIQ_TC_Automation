*** Settings ***
Force Tags  suite
Documentation     PMAUseCase for subnetwork
...
Verify Alarm generation and Data Integrity
    [Tags]  moid
    Log		${EXEC_DIR}
    ${json}=    OperatingSystem.get file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/pm-alarm/PMAClientUsecase_Subnetwork.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    FOR   ${key}   IN  @{object.keys()}
        Add Test Case    ${key}   Verify Alarm Generation for all types of alarm     ${object}[${key}]
    END
    
*** Keywords ***  
Verify Alarm Generation for all types of alarm
    [Arguments]      ${data}
    ...
    ${test_script}=    Get iron_python script for PMA Alarm verification for usecase    ${scripts}\\AlarmRuleCreationUseCase_Subnetwork.py     ${data}		${loc}																																	   
