...
Validate the 'Measure Type': "MultiTable KPI+counter " combination list in Alarm Rule Manager page
    [Tags]  MultiTableKPIcounter
    Log		${EXEC_DIR}
    ${json}=    OperatingSystem.get file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/pm-alarm/PMA_ClientUI.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    Add Test Case    TC01   Validate the Measure Type combination list in Alarm Rule Manager page      ${object}[TC01]
...
Validate the Measure Type combination list in Alarm Rule Manager page
    ...
    ${test_script}=    Get iron_python script for PMA Alarm verification for usecase    ${scripts}\\ApplyAlarmTemplate.py     ${data}		${loc}																																	   
