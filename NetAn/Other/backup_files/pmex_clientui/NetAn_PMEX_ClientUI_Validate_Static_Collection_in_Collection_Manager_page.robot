*** Settings ***
Force Tags  suite
Documentation     PMEx Static Collection Creation Testcase
...
*** Test Cases ***
Verify static collection creation in PMEX client
    [Tags]  pmexClientUI
    ...
    Verify static collection creation in client     ${object}[TC01]
 
    
*** Keywords ***   
Verify static collection creation in client
    [Arguments]      ${data}
    Launch the Tibco spotfire PMEX Application
	${loc}=		Replace String 		${file_loc}	 \\		\\\\
	${test_script}=    Get iron_python script for pmex dynamic node collection manager     ${scripts}\\pmexStaticCollectionCreationVerification.py     ${data}     ${loc}		
	Log 	${test_script}
