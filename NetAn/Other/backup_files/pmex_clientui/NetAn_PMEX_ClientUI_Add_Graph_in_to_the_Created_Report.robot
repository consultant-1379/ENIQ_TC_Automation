...
Force Tags  suite
Documentation     Add graph in to the created report in PMEX client
...
Add graph in to the created report in PMEX client
	...
	Add graph in to the created report      ${object}[TC14]	
...
Add graph in to the created report  
	...
	${test_script}=    Get iron_python script by passing project location for pmex usecase2    ${scripts}\\pmexAddGraphToReport.py     ${data}     ${loc}		MOID
	Log 	${test_script}
    ...
    Verify graph is added      ${ReportDetailsFromClient}		Unique count(Interval Name)per Interval Name_PieChart
