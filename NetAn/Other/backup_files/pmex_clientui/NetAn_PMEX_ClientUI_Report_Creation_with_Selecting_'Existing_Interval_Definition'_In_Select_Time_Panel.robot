*** Settings ***
Force Tags  suite
Documentation     Verify Report Creation with with Selecting 'Existing Interval Definition' In Select Time Panel in PMEX client
...
*** Test Cases ***
Verify Report Creation with with Selecting 'Existing Interval Definition' In Select Time Panel in PMEX client
	...
	Verify report creation      ${object}[TC12]
...
Verify report creation
	...
	${test_script}=    Get iron_python script by passing project location for pmex usecase2    ${scripts}\\pmexReportCreationWithExistingIntervalScript.py     ${data}     ${loc}		MOID
	Log 	${test_script}
