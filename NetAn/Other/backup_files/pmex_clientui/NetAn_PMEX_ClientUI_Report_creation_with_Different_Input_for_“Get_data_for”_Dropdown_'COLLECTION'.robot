*** Settings ***
Force Tags  suite
Documentation     Verify Report Creation with Input 'Get data for' Dropdown 'COLLECTION' in PMEX client
...
*** Test Cases ***
Verify Report Creation with Input 'Get data for' Dropdown 'COLLECTION' in PMEX client
	...
	Verify report creation      ${object}[TC11]
...
Verify report creation
	...
	${test_script}=    Get iron_python script by passing project location for pmex report creation collection    ${scripts}\\pmexReportCreationCollectionScript.py     ${data}     ${loc}		MOID
	Log 	${test_script}
