...
Verify report creation and data integrity in PMEX client for KPI
    [Tags]  dataIntegrity     pmexClientUI
    ${json}=    OperatingSystem.get file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMEXClientUseCase_Legacy.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
	${count}=       set variable     0
    FOR   ${key}   IN  @{object.keys()}
	    IF     '${count}'=='0'
                Run keyword       Verify report creation and Data Integrity for KPI       ${object}[${key}]
                ${count}=       set variable     1
        ELSE				
                Add Test Case    ${key}    run keyword and ignore error   Verify report creation and Data Integrity       ${object}[${key}]
		END        
    END
    
*** Keywords ***

Verify report creation and Data Integrity for KPI
    [Arguments]      ${data}
	...
	Log 	${test_script}
	Execute iron python script for PM Explorer      ${test_script}     False 
