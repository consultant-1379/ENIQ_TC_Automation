*** Settings ***
Documentation     Verify report creation for'Existing Interval Definition', 'No Aggregation' and 'Network'
...
*** Test Cases ***
Verify Report creation in PMEX for 'Existing Interval Definition', 'No Aggregation' and 'Network'
    [Tags]     PMEX_CDB          Report_Manager
    ${json}=    OperatingSystem.get file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMEXWebUI_Existing_Interval_Definition_No_Aggregation_Network.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue On Failure   Verify report creation for'Existing Interval Definition', 'No Aggregation' and 'Network'     ${object}[${key}]
    END    
    
*** keywords ***
Verify report creation for 'Existing Interval Definition', 'No Aggregation' and 'Network'
    [Arguments]      ${data}
