*** Settings ***
Documentation     Verify report creation for'Calendar Interval', 'Node' and 'Collection'
...
Verify Report creation in PMEX with data 'Calendar Interval', 'Node' and 'Collection'
    [Tags]       PMEX      Report_Manager
    ${json}=    OperatingSystem.get file    ./ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMExWebUIReport_CalendarInterval_Node_Collection.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
	${count}=       set variable     0
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue on Failure    Verify report creation for'Calendar Interval', 'Node' and 'Collection'    ${object}[${key}]
...
*** keywords ***
Verify report creation for'Calendar Interval', 'Node' and 'Collection'
    [Arguments]      ${data}
