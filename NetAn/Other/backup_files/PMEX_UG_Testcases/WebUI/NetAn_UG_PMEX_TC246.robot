*** Settings ***
Documentation     Verify report creation for'Existing Interval Definition', 'Node' and 'Nodes'
...
Verify Report creation in PMEX
    ...
    ${json}=    OperatingSystem.get file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMEXWebUI_Existing_Interval_Definition_Node_Nodes.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue On Failure    Verify report creation for'Existing Interval Definition', 'Node' and 'Nodes'     ${object}[${key}]
    END    
    
*** keywords ***
Verify report creation for'Existing Interval Definition', 'Node' and 'Nodes'
    [Arguments]      ${data}
