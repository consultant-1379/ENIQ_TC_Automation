*** Settings ***
Documentation     Verify report creation for'Existing Interval Definition', 'Cell' and 'Nodes'
...
*** Test Cases ***
Verify Report creation in PMEX for'Existing Interval Definition', 'Cell' and 'Nodes'
    [Tags]     PMEX_CDB          Report_Manager
    ${json}=    OperatingSystem.get file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMEXWebUI_Existing_Interval_Definition_Cell_Nodes.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue On Failure    Verify report creation for'Existing Interval Definition', 'Cell' and 'Nodes'     ${object}[${key}]
    END    
    
*** keywords ***
Verify report creation for'Existing Interval Definition', 'Cell' and 'Nodes'
    [Arguments]      ${data}
