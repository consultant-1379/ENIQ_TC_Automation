...
Verify Report creation in PMEX
    ...
    ${json}=    OperatingSystem.get file    ./ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMExWebUIReport_Nodes_CalendarInterval.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
	${count}=       set variable     0
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue on Failure    Report Creation with Input 'Get data for' Dropdown 'Node(s)' and Object Aggregation for 'Cell' with Calendar Interval	${object}[${key}]
...
*** keywords ***
Report Creation with Input 'Get data for' Dropdown 'Node(s)' and Object Aggregation for 'Cell' with Calendar Interval
    [Arguments]      ${data}
