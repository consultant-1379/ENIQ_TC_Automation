*** Settings ***
Documentation     Report creation with different time aggregation input with combination of  Last 30 Days Period_get data for Network_Node aggregation
...
*** Test Cases ***
Create Report with Network, Node aggregation and last 30 days combination
    [Tags]       PMEX           Report_Manager
    open pm explorer analysis
    Click on Report manager button
    ${json}=    OperatingSystem.get file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMEXWebUI_Last30days_Node_Network.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    FOR   ${key}   IN  @{object.keys()}
       Run keyword And Continue On Failure     Report creation for combination of Last 30 Days Period_get data for Network_Node aggregation       ${object}[${key}]

    END


*** keywords ***
Report creation for combination of Last 30 Days Period_get data for Network_Node aggregation
    [Arguments]      ${data} 
	open pm explorer analysis	
    Click on Report manager button 	
