*** Settings ***
Documentation     Report Creation with Input 'Get data for' Dropdown 'SubNetwork' and Object Aggregation for 'NoAggregation' with Calendar Interval
...
Verify Report creation in PMEX with data 'Get data for' Dropdown 'SubNetwork' and Object Aggregation for 'NoAggregation' with Calendar Interval
    [Tags]       PMEX      Report_Manager
    ${json}=    OperatingSystem.get file    ./ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMExWebUIReport_CalendarInterval_NoAggregation_Subnetwork.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
	${count}=       set variable     0
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue on Failure    Report Creation with Input 'Get data for' Dropdown 'SubNetwork' and Object Aggregation for 'NoAggregation' with Calendar Interval	${object}[${key}]
...
*** keywords ***
Report Creation with Input 'Get data for' Dropdown 'SubNetwork' and Object Aggregation for 'NoAggregation' with Calendar Interval
    [Arguments]      ${data}
