*** Settings ***
Documentation     Verify report creation for'Calendar Interval', 'Network' and 'Network'
...
Verify Report creation in PMEX with data 'Calendar Interval', 'Network' and 'Network'
    [Tags]       PMEX      Report_Manager
    ${json}=    OperatingSystem.get file    ./ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMExWebUIReport_CalendarInterval_Network_Network.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
	${count}=       set variable     0
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue on Failure    Verify report creation for'Calendar Interval', 'Network' and 'Network'	${object}[${key}]
...
*** keywords ***
Verify report creation for'Calendar Interval', 'Network' and 'Network'
    [Arguments]      ${data}
