*** Settings ***
Documentation     Validate the ObjectAggregation with Node & GetData For Nodes 
...
Verify Report creation in PMEX  
    ...
    ${json}=    OperatingSystem.get file    ./ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMEXWebUI_Preceding_Period_Node_Nodes.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue On Failure    Validate the ObjectAggregation with Node & GetData For Nodes     ${object}[${key}] 
    END  


*** keywords ***

Validate the ObjectAggregation with Node & GetData For Nodes 
    [Arguments]     ${data}
