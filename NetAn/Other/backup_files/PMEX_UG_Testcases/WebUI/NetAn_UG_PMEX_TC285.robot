*** Settings ***
Documentation     Verify report creation for'Calendar Interval', 'Node' and 'Nodes'
...
Verify Report creation in PMEX with data 'Calendar Interval', 'Node' and 'Nodes'
    [Tags]       PMEX      Report_Manager
    ${json}=    OperatingSystem.get file    ./ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMExWebUIReport_CalendarInterval_Node_Nodes.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
	${count}=       set variable     0
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue on Failure    Verify report creation for'Calendar Interval', 'Node' and 'Nodes'	${object}[${key}]
...
*** keywords ***
Verify report creation for'Calendar Interval', 'Node' and 'Nodes'
    [Arguments]      ${data}
