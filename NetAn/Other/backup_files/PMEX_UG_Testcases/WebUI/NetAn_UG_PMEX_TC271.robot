*** Settings ***
Documentation     Report creation with different time aggregation input with combination of  Last 30 Days Period_get data for Collection_noagg aggregation
...
Create Report with collectin, noagg aggregation and last 30 days combination
    [Tags]       PMEX           Report_Manager        
    ${json}=    OperatingSystem.get file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMEXWebUI_Last30days_Noaggregation_Collection.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    FOR   ${key}   IN  @{object.keys()}
       Run keyword And Continue On Failure     Report creation for combination of Last 30 Days Period_get data for Collection_noagg aggregation       ${object}[${key}]
...
*** keywords ***    
Report creation for combination of Last 30 Days Period_get data for Collection_noagg aggregation
    [Arguments]      ${data}
