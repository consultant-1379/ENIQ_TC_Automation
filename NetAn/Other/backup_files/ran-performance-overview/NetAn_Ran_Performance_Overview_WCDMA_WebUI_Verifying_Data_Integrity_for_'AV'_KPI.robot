...
Verify Data Integrity for The KPI 'Cell availability (AV)'
	${status}=    run keyword and return status    Verify Data Integrity for 'Cell availability (AV)' KPI
	Log    ${status}

*** Keywords ***

Verify Data Integrity for 'Cell availability (AV)' KPI
	...
	select a QoS KPI    Cell availability (AV)
	...
	${sql_query}=    get sql query from json file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/ran-performance-overview/RPO_dataIntegrity.json     AV_Today
	...
	${sql_query1}=    get sql query from json file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/ran-performance-overview/RPO_dataIntegrity.json     AV_PastWeek
