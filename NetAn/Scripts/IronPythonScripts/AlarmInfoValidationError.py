...
class TestGetValidationErrorScript(unittest.TestCase):    
    def test_get_validation_Error_msg(self): 
        run_read_specificPrblmErrorMsg()					
...
def run_read_specificPrblmErrorMsg():
    global alarmTitle
    errorMessage = Document.Properties["ValidationError"]
    print("errorMessage:")
    print(errorMessage)
...
def pma_testing_suite():
    ...
    if testing_level == 'Full':
        ...
        test_suite.addTest(unittest.makeSuite(TestGetValidationErrorScript))        
        time.sleep(30)
