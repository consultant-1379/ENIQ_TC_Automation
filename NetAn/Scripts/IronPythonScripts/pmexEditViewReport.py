...
class TestGetViewPageNameScript(unittest.TestCase):
    def test_edit_pageName(self):
        time.sleep(30)
        viewPageName=Document.Properties["ActivePageName"]  
        print(viewPageName)   
        fileloc = os.path.join("PROJ_LOC" ,"reportDetails.txt")                                              
        f = open(fileloc, 'w')
        f.truncate(0)        
        time.sleep(5)
        f.write(viewPageName)
        time.sleep(5)        
        
    def tearDown(self):
        clean_environment()

class TestGetEditPageNameScript(unittest.TestCase):
    def test_edit_pageName(self):
        time.sleep(30)
        editPageName=Document.Properties["ActivePageName"]  
        print(editPageName)   
        fileloc = os.path.join("PROJ_LOC" ,"title.txt")                                              
        f1 = open(fileloc, 'w')
        f1.truncate(0)        
        time.sleep(5)
        f1.write(editPageName)
        time.sleep(5)        
...
class TestCloseReportClickScript(unittest.TestCase):    
    def test_edit_report(self):  
        run_close_report()        
...
def run_close_report():
    scriptDef = clr.Reference[ScriptDefinition]()
    Document.ScriptManager.TryGetScript("CloseReport", scriptDef)
    paramDict = {}
...
def run_select_columnRow(): 
    ...
    fileloc = os.path.join("PROJ_LOC" ,"PMDataColumnValue.txt")                                              
    f = open(fileloc, 'w')
    f.truncate(0)     
    for eColumn in table.Columns:
            for eTableRows in range(0,1):
                    colValue="\tColumn: " + eColumn.Name + "\tValue: " +table.Columns[eColumn.Name].RowValues.GetFormattedValue(eTableRows)
                    print "\tColumn: " + eColumn.Name + "\tValue: " +table.Columns[eColumn.Name].RowValues.GetFormattedValue(eTableRows)
                    f.write(colValue)
    f.close
...
def run_select_reportname():
    ...
    cursor2 = DataValueCursor.CreateFormatted(table.Columns["ReportTableList"])
...
def run_select_and_get_reportname():
    #get the data table
    table=Document.Data.Tables["tblSavedReports"]
...
def pmex_testing_suite():
    ...
    if testing_level == 'Full':
        ...
        test_suite.addTest(unittest.makeSuite(TestGetEditPageNameScript))         
        test_suite.addTest(unittest.makeSuite(TestCloseReportClickScript))
        test_suite.addTest(unittest.makeSuite(TestSelectReportWithReportNameScript))
        test_suite.addTest(unittest.makeSuite(TestViewReportScript))
        test_suite.addTest(unittest.makeSuite(TestGetViewPageNameScript))        
        test_suite.addTest(unittest.makeSuite(TestCloseReportClickScript))    
