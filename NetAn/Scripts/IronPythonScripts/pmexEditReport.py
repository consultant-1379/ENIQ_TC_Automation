...
params_for_save_report_library = [[
 {
   ...
   "Type": "String",
   #"Value": "Test2022Feb"
   "Value": "testEdit"+(datetime.now()).strftime("%d%b%H%M")
 },
 {
...
class TestSelectReportWithReportNameAfterEditScript(unittest.TestCase):    
    def test_select_row(self):  
        run_select_reportname_edit()
...
class TestGetReportNamesScript(unittest.TestCase):    
    def test_get_reportNames(self):  
        run_get_reportnames()
...
def run_select_reportname():
    ...
    cursor2 = DataValueCursor.CreateFormatted(table.Columns["ReportDescription"])
    ...
    for row in table.GetRows(cursor,cursor2):
            value = cursor.CurrentValue
            if value == reportName:
                rowIndexOfReport = row.Index
                valData1=cursor2.CurrentValue
                #f.write(valData1)
                #time.sleep(5)
                break                                                  
...
def run_select_reportname_edit():
    #get the data table
    table=Document.Data.Tables["tblSavedReports"]
...
def pmex_testing_suite():
    ...
    if testing_level == 'Full':
        ...
        test_suite.addTest(unittest.makeSuite(TestSelectReportWithReportNameAfterEditScript))
        test_suite.addTest(unittest.makeSuite(TestGetReportNamesScript))            
    else:
        pass
