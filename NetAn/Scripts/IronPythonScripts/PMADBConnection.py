...
class TestAddMeasuresScript2(unittest.TestCase):
    def test_add_measures(self):
        ...
        setMarking(tableName, columnName, markingName, dataToMark = KPI_NAME)
        run_add_measures()
...
def run_select_row():
    ...
    table=Document.Data.Tables[alarmName]

    
    #place generic data cursor on alarmName specific column
    cursor = DataValueCursor.CreateFormatted(table.Columns["MEASUREVALUE_1"])
    ...
    cursor4 = DataValueCursor.CreateFormatted(table.Columns["DATE_ID"])
    date = ''
    #iterate through table column rows to retrieve the values
    for row in table.GetRows(cursor4):
        value3 = cursor4.CurrentValue
        rowIndexOfReport = row.Index
        split_string = value3.split(" ",1)
        date=split_string[0]
        break
    print("date:")
    print(date)
