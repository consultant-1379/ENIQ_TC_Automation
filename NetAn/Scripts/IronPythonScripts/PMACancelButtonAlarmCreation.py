...
class TestGetAlarmListScript(unittest.TestCase):    
    def test_select_row(self):  
        run_read_rowValues()
...
class TestCancelButtonScript(unittest.TestCase):    
    def test_create_button_click(self):  
        run_cancel_button_click()
...
def run_cancel_button_click():
   
    scriptDef = clr.Reference[ScriptDefinition]()
    Document.ScriptManager.TryGetScript("CancelAndCleanUp", scriptDef)
    paramDict = {} 
...
def run_read_rowValues(): 
    ...
    filelocn = os.path.join("PROJ_LOC" ,"alarmList.txt")
    f3 = open(filelocn, 'w')
    f3.truncate(0)	
...
def pma_testing_suite():
    ...
    if testing_level == 'Full':
	    ...
	    time.sleep(30)
	    test_suite.addTest(unittest.makeSuite(TestCancelButtonScript))
	    time.sleep(20)
	    test_suite.addTest(unittest.makeSuite(TestGetAlarmListScript))													 
    else:
        pass
