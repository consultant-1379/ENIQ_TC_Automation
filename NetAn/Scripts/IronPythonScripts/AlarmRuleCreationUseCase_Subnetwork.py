...
class TestSelectNodesScript(unittest.TestCase):
    def test_select_nodes(self):
        ...
        filterName = "filteredschemesubnetwork"
        columnName = "Filtersubnetwork"     
        #variable
        filterValues = NODE
...
class TestReadValuesOfFetchDataScript(unittest.TestCase):    
    def test_select_row(self):
        ...
        time.sleep(10)
        run_select_alarm_criteria_values()        
...
class TestRunSupportingActivateScript(unittest.TestCase):    
    def test_run_supp_script(self): 
        run_supp_activate_script()
...
def run_read_rowValues(): 
    ...
    rowsToMark.AddIndex(rowIndexOfReport)
    Document.ActiveMarkingSelectionReference.SetSelection(RowSelection(rowsToMark),table)
    state=table.Columns['AlarmState'].RowValues.GetFormattedValue(rowIndexOfReport)
    print state
    filelocn = os.path.join("PROJ_LOC" ,"alarmState.txt")
    f3 = open(filelocn, 'w')
    f3.truncate(0)																																			   					  			  
    time.sleep(2)													   
    f3.write(state)
...
def run_select_alarm_criteria_values():
    
    table=Document.Data.Tables[alarmTitle]
    fileloc = os.path.join("PROJ_LOC" ,"AlarmCriteria.txt")
    f = open(fileloc, 'w')
    f.truncate(0) 
    cursor = DataValueCursor.CreateFormatted(table.Columns["ALARM_CRITERIA"])
    for row in table.GetRows(cursor):
            value = cursor.CurrentValue
            colValue=value + "," 
            f.write(colValue)
...
def pma_testing_suite():
    ...
    if testing_level == 'Full':
	    ...
	    test_suite.addTest(unittest.makeSuite(TestActivateScript))
	    time.sleep(5)
