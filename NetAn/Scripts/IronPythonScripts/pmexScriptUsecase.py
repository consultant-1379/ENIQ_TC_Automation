...
def run_select_row():
	...
	cursor = DataValueCursor.CreateFormatted(table.Columns["KPI_USED"])
	...
	cursor2 = DataValueCursor.CreateFormatted(table.Columns["DATA_OF_COL1"])
	moid = ''
	#iterate through table column rows to retrieve the values
	...
	print("moid:")
	print(moid)
	fileloc = os.path.join("PROJ_LOC" ,"dataOfCol1.txt")
	f = open(fileloc, 'w')
	f.truncate(0) 
	f.write(moid)
	f.close
	
	cursor4 = DataValueCursor.CreateFormatted(table.Columns["DATA_OF_COL2"])
	moid = ''
	#iterate through table column rows to retrieve the values
	for row in table.GetRows(cursor4):
		value4 = cursor4.CurrentValue
		rowIndexOfReport = row.Index
		moid=value4
		break
	print("moid:")
	print(moid)
	fileloc = os.path.join("PROJ_LOC" ,"dataOfCol2.txt")
	f = open(fileloc, 'w')
	f.truncate(0) 
	f.write(moid)
	f.close
	
	cursor3 = DataValueCursor.CreateFormatted(table.Columns["DATE_ID"])
	date = ''
	#iterate through table column rows to retrieve the values
