...
class TestNavToTimeSelScript(unittest.TestCase):
    def test_time_selection(self):
        ...
        time.sleep(3)
        intervalEndDt=Document.Properties["IntervalEndDate"]	
        fileloc = os.path.join("PROJ_LOC" ,"dataOfCol1.txt")
        f = open(fileloc, 'w')
        f.truncate(0) 
        f.write(intervalEndDt)	
        time.sleep(5)
        f.close      
        run_add_interval()
...
def run_select_datetime_interval_definitions():
    ...
    for row in table.GetRows(cursor):
            ...
            if value == interval_name:
                ...
                endtime=table.Columns["End Time"].RowValues.GetFormattedValue(rowIndexOfReport)
                fileloc = os.path.join("C:\\Users\\Administrator2\\Shimmy_Admin2\\pmaTR\\ENIQ_TC_Automation\\NetAn\\Other\\Files" ,"dataOfCol2.txt")
                f = open(fileloc, 'w')
                f.truncate(0) 
                f.write(endtime)	
                time.sleep(5)
                f.close  		 
                print("endtimefromselct")                
                print(endtime)                
                valData1=value
                break
...
def run_select_columnRow():
    ...
    cursor2 = DataValueCursor.CreateFormatted(table.Columns["MONTH_ID"])
    fileloc = os.path.join("C:\\Users\\Administrator2\\Shimmy_Admin2\\pmaTR\\ENIQ_TC_Automation\\NetAn\\Other\\Files" ,"PMDataColumnValue.txt")                                              
    f = open(fileloc, 'w')
    f.truncate(0) 
    monthid =''
