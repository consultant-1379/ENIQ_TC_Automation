...
params_for_save_report_library = [[
 {
   ...
   "Type": "String",
   #"Value": "Test2022Feb"
   "Value": "testGraph"+(datetime.now()).strftime("%d%b%H%M")
 },
 {
...
params_for_save_report_library_edit = [[
 {
   "Name": "ReportDescription",
   "Type": "String",
   "Value": "AddedGraph"
...
class TestNavigateToSaveAfterEditScript(unittest.TestCase):
    def test_navTo_save(self):
        #set_test_case_from_json(params_for_fetch_nodes)   
        run_navTo_save()
...
class TestSelectColumnNamesWithValueOfPMDataScript(unittest.TestCase):    
    def test_select_row(self):  
        run_select_columnRow()
...
class TestCheckDataIntegrityCheckBoxScript(unittest.TestCase):    
    def test_checkbox_check(self):
        checkbox = 'CHECK_BOX' 
        if    checkbox=='yes':
            FilterSelection = Document.Data.Filterings["OperationsFilteringScheme"]
            dataIntegrity = Document.FilteringSchemes[FilterSelection][Document.Data.Tables["OperationsLists"]]["OperationsForFetch"].As[filters.CheckBoxFilter]()
            for CheckBoxValue in dataIntegrity.Values:
                     dataIntegrity.Uncheck(CheckBoxValue)
                     dataIntegrity.Check(CheckBoxValue)
            time.sleep(7) 
            run_dataIntegrity_checkbox()
            #Document.Properties["DataAvailSwitch"] = "ON"
            checkboxStatus = Document.Properties["DataAvailSwitch"]
...
class TestEditReportScript(unittest.TestCase):    
...
class TestSaveReportToLibraryAfterEditScript(unittest.TestCase):
    def test_save_report(self):
        set_test_case_from_json(params_for_save_report_library_edit)   
        global reportDesc
        reportDesc=Document.Properties["ReportDescription"]  
        print(reportDesc)                                                              
        run_save_report()
...
class TestAddGraphScript(unittest.TestCase):    
    def test_add_graph(self):  
        myVis = Document.ActivePageReference.Visuals.AddNew[PieChart]()
        table=Document.Data.Tables.Item["Datetime Interval Definitions"]         
        myVis.Data.DataTableReference=table        
        myVis.VisualAttributes.SortSectorsBySize = True
        myVis.Title = 'Unique count(Interval Name)per Interval Name'
        time.sleep(20)                                                                   
...
def run_dataIntegrity_checkbox():
    scriptDef = clr.Reference[ScriptDefinition]()
    Document.ScriptManager.TryGetScript("setDataAvailability", scriptDef)
    paramDict = {} 
...
def run_select_reportname():
    ...
    Document.Properties["SelectedReport"]=reportName
    print(Document.Properties["SelectedReport"])                      
...
def pmex_testing_suite():
    ...
    if testing_level == 'Full':
        ...
        test_suite.addTest(unittest.makeSuite(TestCheckDataIntegrityCheckBoxScript))        
        ...
        test_suite.addTest(unittest.makeSuite(TestSelectColumnNamesWithValueOfPMDataScript))                                                                                    
        time.sleep(30)
        ...
        time.sleep(20)
        test_suite.addTest(unittest.makeSuite(TestEditReportScript)) 
        time.sleep(60)              
        test_suite.addTest(unittest.makeSuite(TestAddGraphScript))
        time.sleep(50)              
        test_suite.addTest(unittest.makeSuite(TestNavigateToSaveAfterEditScript))
        test_suite.addTest(unittest.makeSuite(TestSaveReportToLibraryAfterEditScript))  
        test_suite.addTest(unittest.makeSuite(TestSelectReportWithReportNameScript))         
    else:
        pass
