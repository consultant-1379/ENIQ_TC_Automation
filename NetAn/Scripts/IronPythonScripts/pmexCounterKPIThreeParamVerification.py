...
def run_select_row():
    ...
    cursor3 = DataValueCursor.CreateFormatted(table.Columns["NODE_NAME"])
    nodename = ''
    #iterate through table column rows to retrieve the values
    ...
    print("nodename:")
    print(nodename)
    fileloc = os.path.join("PROJ_LOC" ,"NRCellDUvalue.txt")
    f = open(fileloc, 'w')
    f.truncate(0) 
    f.write(date)
    f.close
    
    cursor4 = DataValueCursor.CreateFormatted(table.Columns["DATETIME_ID"])
    date = ''
    #iterate through table column rows to retrieve the values
