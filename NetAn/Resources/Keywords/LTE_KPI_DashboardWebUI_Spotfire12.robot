...
open lte kpi dashboard analysis
    ...
    Sleep    3
    Wait Until element is visible      xpath: (//input[@class="w-full pt-2 pb-2 pl-3 text-ellipsis pr-3 ng-untouched ng-pristine ng-invalid"])[1]    30
    ...
    Wait Until element is visible      xpath: /html/body/tss-root/div/div/tss-login/div/div/form/tss-input[2]/div/div/input    30
    Selenium2Library.Input Text   xpath: /html/body/tss-root/div/div/tss-login/div/div/form/tss-input[2]/div/div/input    Ericsson01
    Wait Until element is visible      xpath: /html/body/tss-root/div/div/tss-login/div/div/form/button    30
...
verify the page title  
	...
	wait for page to load
    ${text}=    Selenium2Library.Get text  xpath: (//div[@class="sfx_page-title_375"])
    Log    ${text}
...
go to home page if not at home
	wait for page to load
	${pageTitle}=    Selenium2Library.Get text  xpath: (//div[@class="sfx_page-title_375"])
	IF 		"${pageTitle}" != "Home"
...
Match UI with DB Value
	...
	Log    ${dbValue1}
	verify that the DB and UI values match    ${uiValue}     ${dbValue1}
...
Make selection on worst performing cells(latest rop) chart
	...
	wait until element is visible    xpath: //div[@class="sfx_label_379"][2]    300
	sleep    1
	...
	IF    "${value}"=="0 marked"
		...
		wait until element is visible    xpath: //div[@class="sfx_label_379"][2]    300
		sleep    1
		${value1}=    Selenium2Library.get text    xpath: //div[@class="sfx_label_379"][2]
		IF    "${value1}"=="0 marked"
			Wait Until Element Is Visible      xpath: (//div[@class="sf-element-canvas-image"])[2]    300
			Selenium2Library.Drag And Drop by Offset 	xpath: (//div[@class="sf-element-canvas-image"])[2]    -160    -100
			wait until element is visible    xpath: //div[@class="sfx_label_379"][2]    300
			${value2}=    Selenium2Library.get text    xpath: //div[@class="sfx_label_379"][2]
			IF    "${value2}"=="0 marked"
				Wait Until Element Is Visible      xpath: (//div[@class="sf-element-canvas-image"])[2]    300
				Selenium2Library.Drag And Drop by Offset 	xpath: (//div[@class="sf-element-canvas-image"])[2]    -160    -100
			END
...
verify that the difference between the two ROP values is correct
	...
	Match UI with DB Value after approximation    ${differenceValue}    ${uiValue}
	wait for page to load
	capture page screenshot