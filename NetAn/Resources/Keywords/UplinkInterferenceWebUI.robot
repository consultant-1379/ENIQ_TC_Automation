...
reading values from the tooltip in chart
	...
	IF    "${chartName}" == "PRB Analysis"
    	selenium2library.mouse over    xpath:(//div[@class="sf-element-canvas-image sfc-transition-background"])[3]
    ELSE IF    "${chartName}" == "Antenna Analysis"
    	selenium2library.mouse over    xpath:(//div[@class="sf-element-canvas-image sfc-transition-background"])[1]
    ELSE IF    "${chartName}" == "PUCCH Analysis"
    	selenium2library.mouse over    xpath:(//div[@class="sf-element-canvas-image sfc-transition-background"])[5]
    ELSE IF    "${chartName}" == "PUSCH Analysis"
    	selenium2library.mouse over    xpath:(//div[@class="sf-element-canvas-image sfc-transition-background"])[2]
    END
    sleep    5
...
verify that Band and Frequency check box is visible
	element should be visible    xpath:(//div[@class="flex-item flex-container-horizontal flex-inline-container flex-align-center"])[1]
	wait for page to load
	capture page screenshot
	
verify that Channel Bandwidth check box is visible
	element should be visible    xpath:(//div[@class="flex-item flex-container-horizontal flex-inline-container flex-align-center"])[2]
	wait for page to load
	capture page screenshot
