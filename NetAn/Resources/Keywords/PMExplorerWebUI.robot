...
change to page navigation to
	[Arguments]    ${mode}
	Wait Until Page Contains Element      xpath:(//div[@class="sfx_page-title_219"])     timeout=300
	FOR    ${i}    IN RANGE    0     10
         ${status}=      Run keyword and return status   element should be visible      xpath://*[@title="Add new page"]
         IF   ${status} is ${TRUE}
               Exit For Loop
         ELSE
		        Run keyword and ignore error     open context menu      xpath:(//div[@class="sfx_page-title_219"])
				Sleep    2s
...
Select KPIs
    ...
    FOR    ${kpi}    IN    @{list}
            ...
            click on scroll down button       9        1
            wait until element is visible     xpath://div[@name='valueCellCanvas']//div[@row='1' and @column='0']   120          
			...
			sleep    2s
		    ${marked}=    selenium2library.get text    xpath: (//div[@class="sfx_label_223"])[2]
		    IF    "${marked}"=="0 marked"
				FOR    ${i}    IN RANGE    0     4
...
wait for page to load
	Sleep   3s
	run keyword and ignore error       Wait Until Element Is Not Visible      xpath://*[@class='sf-svg-loader-12x12']              timeout=500
    run keyword and ignore error       Wait Until Element Is Not Visible     class:sfx_progress-bar-container_1118       timeout=1500
    Sleep   2s
    run keyword and ignore error       Wait Until Element Is Not Visible     class:sfx_progress-bar-container_1118       timeout=1500
    Sleep   2s
	run keyword and ignore error       Wait Until Element Is Not Visible      xpath://*[@class='sf-svg-loader-12x12']              timeout=500
...
Select no subnetwork
    capture page screenshot
	${status}=    run keyword and return status    Click Element       xpath://div[@title='(Empty)']
	IF    "${status}"=="False"
		FOR    ${i}    IN RANGE    0    200
			place cursor on    Start Time
			Click on scroll down button    6    12
			${status}=    run keyword and return status    Element should be visible    xpath://div[@title='(Empty)']
			Exit For Loop If     ${status} == True
		END		
    Click Element      xpath://div[@title='(Empty)']
	capture page screenshot		
	END
	sleep    2s
	Click Element      xpath://div[@title='(Empty)']      modifier=CTRL
	wait for page to load
	capture page screenshot
...
Select Collection
    ...
    @{list}=      Split string      ${node_list}    ,
     FOR    ${node}    IN    @{list}
           Clear Element Text      xpath://html/body/div[1]/div[2]/div/div[1]/div/div/div[2]/div/div/div/div/div/div/div/div/div[3]/div/div[1]/div[1]/span[4]/div[2]/p/span/div[1]/input
           sleep    5s
		   Selenium2Library.Input Text     xpath://html/body/div[1]/div[2]/div/div[1]/div/div/div[2]/div/div/div/div/div/div/div/div/div[3]/div/div[1]/div[1]/span[4]/div[2]/p/span/div[1]/input     ${node} 
           Click on scroll down button    6     1
		   sleep    2s
           wait for page to load
           Click element      xpath://div[@title='${node}']      modifier=CTRL
...
select SubNetwork
	...
	Click on scroll down button     6    21
     FOR    ${subNet}    IN    @{subNetwork}
           Clear Element Text      xpath://*[@id="8447faec751d4048a47f1ab93c521944"]/div[1]/input
           Selenium2Library.Input Text     xpath://*[@id="8447faec751d4048a47f1ab93c521944"]/div[1]/input     ${subNet} 
           press key      xpath://*[@id="8447faec751d4048a47f1ab93c521944"]/div[1]/input     \\13
           sleep    2s
           wait for page to load
...
Change the View to
	...
	Wait Until Page Contains Element      xpath://div[text()="${view}" and @class="sfx_label_936"]     timeout=150	
    Sleep      10s	
...
Change the Visualization type to Pie chart
	...
	sleep    5
	Wait Until Page Contains Element      xpath://div[@class="sfx_button_303 sfx_button-enabled_302"][7]     timeout=15
	Click Element      xpath://div[@class="sfx_button_303 sfx_button-enabled_302"][7]
...
select the created report
    ...
    press key      xpath://div[@class='HtmlTextArea sf-enable-selection sf-focusable']      \\13
    Sleep    5s
	Wait Until Page Contains Element      xpath://*[contains(text(),'${report}')]                1500
...
check the show in user interface for tables
	...
	Sleep     20s
	Click Element    xpath:(//div[@class="sf-element sf-element-icon"])
	${command} =  Run Keyword And Return Status    Element Should Not Be Visible      xpath:(//div[@title="${table}"])
	Run Keyword if      ${command}      Run keyword and ignore error       Click on the scroll down button       1      ${click}
	Run Keyword if      ${command}      Run keyword and ignore error       Click on the scroll down button       2      ${click}
	Click Element    xpath:(//div[@title="${table}"])
	Sleep      5s	
	${command} =  Run Keyword And Return Status    Element Should Not Be Visible      xpath://div[@class="sfx_checkbox_930 sfx_checked_931"]
	Run Keyword if      ${command}       Click Element    xpath:(//div[@class="sfx_checkbox_930"])[1]
	
	Sleep      5s
	Click Element    xpath:(//div[@class="flex-item flex-align-start sfx_DataCanvasToolbarButtonIcon_50"])[4]
	Sleep      5s
	wait for page to load
	capture page screenshot										   
...
select a column to add to the report
	...
	Click Element     xpath:(//div[@title = "Data canvas"])
	sleep    5
	Wait Until Page Contains Element      xpath:(//div[@class="sfx_dropdown_772 sf-element-dropdown"])     timeout=1500
    Click Element    xpath:(//div[@class="sfx_dropdown_772 sf-element-dropdown"])
    sleep    3
	Click on scroll down button    4     40
    Wait Until Page Contains Element      xpath:(//div[@class="ListItems"])//div[contains(text(),'pm_DC_E_TCU_ACLENTRYIP6_DAY(No Aggregation Level)__NetAn_ODBC')][1]     timeout=1500
    Click Element    xpath:(//div[@class="ListItems"])//div[contains(text(),'pm_DC_E_TCU_ACLENTRYIP6_DAY(No Aggregation Level)__NetAn_ODBC')][1]
	wait for page to load
	capture page screenshot	
	
add a calculated column
	Wait Until Page Contains Element      xpath:(//*[@class="sfx_buttonSvg_87"])[2]     timeout=15
    Click Element    xpath:(//*[@class="sfx_buttonSvg_87"])[2]
    sleep    3
    click on the button    Add columns
    click on the button    Other
    Sleep     2s
    FOR    ${i}    IN RANGE    0     30
         ${status}=     Run Keyword And Return Status        Element should be visible         xpath:(//*[@title="pm_DC_E_TCU_ACLENTRYIP4_DAY(No Aggregation Level)__NetAn_ODBC"])
         IF   ${status} is ${TRUE}
               Exit For Loop
         ELSE
               Run keyword if      ${status}==False      Selenium2Library.Drag And Drop By Offset      xpath:(//*[@class="sfx_handle-holder_1043"])[5]      0      70
         END
         
    END
    Sleep      2s  
    Click Element     xpath:(//*[@title="pm_DC_E_TCU_ACLENTRYIP4_DAY(No Aggregation Level)__NetAn_ODBC"])
    Sleep      2s
    ...
    sleep    5  
    Wait Until Page Contains Element      xpath:(//div[@class="flex-item flex-align-start sfx_DataCanvasToolbarButtonIcon_50"])[4]    timeout=15
    Click Element    xpath:(//div[@class="flex-item flex-align-start sfx_DataCanvasToolbarButtonIcon_50"])[4]
    wait for page to load
    capture page screenshot
...
Verify if fetch pm data button is enabled
    sleep    5s
    ${status1}=      Run keyword and return status       Click element     xpath: //input[@value='Fetch PM Data']
	Run keyword IF     ${status1}==False     PASS      msg= Fetch nodes button is disabled
    wait for page to load
    capture page screenshot
...
Select KPIs and verify fetch pm data button
    ...
    FOR    ${kpi}    IN    @{list}
           ...
           Click element      xpath://input[@value='>>'] 
           sleep   5s
	       element should be visible    xpath:(//input[@value='Fetch PM Data'])[1]
           wait for page to load
	   Capture page screenshot
     END
...
Verify if all fields are same while saving in edit mode
	...
	element should be visible    xpath://*[contains(text(),'${description}')]
	Click on scroll up button    3    4	
	wait for page to load
	capture page screenshot
...
click on cancel button for sync with eniq
     Wait Until Element Is Visible         class:sfx_progress-bar-container_1118       timeout=3000
...
Open the information link
	...
	Click Element       xpath://div[@class='lp-app-card lp-app-card--analytics']
    Sleep      5
...
Check whether the given value is present in the DCVECTOR_INDEX column
	...
	IF       ${check}==0		
		Wait Until Element Is Not Visible    xpath:(//div[@class="valueCellCanvas"])//div//div[@column="1"]//div[contains(text(),'${bin}')]    150          150
	ELSE
		Wait Until Element Is Visible    xpath:(//div[@class="valueCellCanvas"])//div//div[@column="1"]//div[contains(text(),'${bin}')]    150          150
	END	  
...
Verify the row counts to be same as given in measure selection page
	...
	${text1}=      Get text      xpath:(//div[@class="sfx_label_223"])[1]
	Log      ${text1}
...
Check for the error notification is not present
	...
	Sleep    3s
	${notification}=    Get text    xpath://div[@class='sfx_notification-panel-empty_416']
