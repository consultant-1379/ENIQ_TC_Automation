...
verify error message should be displayed after add button
	${text}=    selenium2library.get text    xpath://span[@id="a9de9873b8fa4757b3dc8a139465033f"]
	Log    ${text}
	should contain    ${text}    Node is already excluded
	wait for page to load
	capture page screenshot
...
Edit Text in Existing Value
    ...
    ELSE IF    '${value}' == "WhereClause"
        sleep    5s
        Click on the scroll down button    2    20
...
select multiple rules in cm rule page
	selenium2library.drag and drop by offset    xpath:(//div[@class="sf-element sf-element-visual-content sfc-table"])    100    80	
	wait for page to load
	capture page screenshot
