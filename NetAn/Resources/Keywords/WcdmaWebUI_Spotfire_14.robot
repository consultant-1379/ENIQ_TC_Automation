...
read the marked value
	${markedValue}=    Selenium2Library.get text 	xpath://div[@class="${sfx_label}"][2]
	[Return]    ${markedValue}
	wait for page to load
	capture page screenshot
...
verify that the marked value is not equal to
	...
	${newMarked}=    Selenium2Library.get text 	xpath://div[@class="${sfx_label}"][2]
...
open wcdma overview analysis
    ...
    wait for page to load
	Wait Until Page Contains Element      xpath: (//input[@value="Play Dashboard"])     timeout=1500
    capture page screenshot

Mouse scroller
    Sleep    5
	Set Window Size      811     651
	Sleep    5
	Maximize Browser Window
	Sleep    5 			  
...
select a QoS KPI
	[Arguments]    ${kpi}
	Click on the scroll down button    2    2
...
read the nodeName and measureValue
	...
	sleep    3s
	${status}=     Run keyword and return status      Element should be visible    xpath://div[@class="sf-tooltip sf-plot-tooltip RoundedDropShadow"]
	IF  ${status}==False
	    Mouse Move    ${IMAGE_DIR}\\RPO_Read_KPI_Values.PNG
	    sleep    3s
	END 
...
verify favorites is displayed with KPIs in settings page
    ${text}=         Selenium2library.Get text        xpath:(//div[@class='HtmlTextArea sf-enable-selection sf-focusable'])[2]
    Log        ${text} 
    Should contain         ${text}          Favorites:
...
Verify the colour of the displayed value
	[Arguments]      ${Element_id}        ${Scroll_element}
	click on the scroll down button    ${Scroll_element}    15
	sleep    5s
	${status}=     Run keyword and return status      Element should be visible     xpath://*[@id="${Element_id}"]
	IF  ${status}==False
	    Click on the scroll up button    ${Scroll_element}    3
	END      
	sleep     5s
	capture page screenshot
	${status}=    selenium2library.get text    xpath://*[@id="${Element_id}"]
	Log    ${status}
	${tem}=     Replace String    ${status}      +       0
	${spl}=    split string    ${tem}    .
	Log    ${spl}
	${val}=    Get From List    ${spl}    0
    ${intst}=     Convert To Integer     ${val}
	Log    ${intst}
    IF    ${intst} >0
		element should be visible      xpath: //*[@id="${Element_id}" and contains(@style,'color: rgb(165, 199, 83);')]
    ELSE IF    ${intst} ==0
		element should be visible      xpath: //*[@id="${Element_id}" and contains(@style,'color: rgb(217, 218, 219);')]
    ELSE
		element should be visible      xpath: //*[@id="${Element_id}" and contains(@style,'color: rgb(233, 92, 56);')]
	END
    capture page screenshot
...
Match UI with DB RPO Value
	...
	Log       ${DB_Value_num}
	${UI_Value_num}=      Convert To Number	     ${uiValue}       0
	Log       ${UI_Value_num}
	...
	#Log    ${dbValue1}
	Compare UI and DB values    ${UI_Value_num}     ${DB_Value_num}
...
read the nodeName and measureValue from current week chart
	#Mouse Move    ${IMAGE_DIR}\\RPO_Read_KPI_Values.PNG
	Mouse over   xpath:(//div[@class="sf-element sf-element-canvas-visualization"])[2]
...
read the nodeName and measureValue from last week chart
	#Mouse over    ${IMAGE_DIR}\\RPO_Read_Pastweek_KPI.PNG
	Mouse over    xpath:(//div[@class="sf-element sf-element-canvas-visualization"])[3]
	sleep    3
	${data}=    Selenium2Library.Get text    xpath://div[@class="sf-tooltip sf-plot-tooltip RoundedDropShadow"]
	Log    ${data}
	${splitData}=    split string    ${data}    :
...
read the nodeName and measureValue from current week Mobility and Accessibility chart
	#SikuliLibrary.Mouse over    ${IMAGE_DIR}\\RPO_Mobility_KPI.PNG
	Mouse over    xpath:(//div[@class="sf-element sf-element-canvas-visualization"])[1]
	sleep    5
	${data}=    Selenium2Library.Get text    xpath://div[@class="sf-tooltip sf-plot-tooltip RoundedDropShadow"]
	Log    ${data}
	${splitData}=    split string    ${data}    :
...
Suite setup steps for wcdma
    Set Screenshot Directory   ./Screenshots
	Set Selenium Implicit Wait        60s
	
Suite teardown steps for wcdma
    Close Browser
	
Test teardown steps for wcdma
    Capture page screenshot
    Close Browser	
