*** Variables ***
${username_xpath}      (//input[@class="w-full pt-2 pb-2 pl-3 text-ellipsis pr-3 ng-untouched ng-pristine ng-invalid"])[1]
${password_xpath}        /html/body/tss-root/div/div/tss-login/div/div/form/tss-input[2]/div/div/input
${loginButton_xpath}      /html/body/tss-root/div/div/tss-login/div/div/form/button
${sfx_label}                sfx_label_379
${sfx_page_title}         (//div[@class="sfx_page-title_375"])
${author_dropdown}        //div[@class='sfx_author-dropdown_586']
${save_analysis_button}           (//div[text()='Save' and @class='sfx_button-text_663'])[2]
${visualisations_type}           sfx_button_336 sfx_button-enabled_335
${sfx_progress-bar}              sfx_progress-bar-container_1369
${analytics_page}               //div[@class='lp-app-card lp-app-card--analytics']
${page_tab}                     sfx_page-tab_360 sf-element-page-tab
${notification_container}                sfx_notification-container_491
...
check the show in user interface for tables
	...
	Sleep     20s
	Click Element    xpath://div[@class='flex-item flex-container-horizontal flex-grow flex-align-center sfx_dropdown_6']
	...
	Sleep      5s
	${command} =  Run Keyword And Return Status    Element Should Not Be Visible      xpath://div[@class="sfx_checkbox_1047 sfx_checked_1048"]
	Run Keyword if      ${command}       Click Element    xpath:(//div[@class="sfx_checkbox_1047"])[1]
	Sleep      5s
	Click Element    xpath:(//div[@class="flex-item flex-align-start sfx_DataCanvasToolbarButtonIcon_1341"])[5]
	Sleep      5s
	wait for page to load
	capture page screenshot	 ccxqza		

select a column to add to the report
	Wait Until Page Contains Element     xpath:(//div[@title = "Data canvas"])     timeout=1500
	Click Element     xpath:(//div[@title = "Data canvas"])
	sleep    5
	Wait Until Page Contains Element      xpath:(//div[@class="flex-item flex-container-horizontal flex-grow flex-align-center sfx_dropdown_6"])     timeout=1500
    Click Element    xpath:(//div[@class="flex-item flex-container-horizontal flex-grow flex-align-center sfx_dropdown_6"])
    sleep    3
	Click on scroll down button    4     30
...
add a calculated column
	Wait Until Page Contains Element      xpath:(//*[@class="sfx_buttonSvg_1250"])[2]     timeout=15
    Click Element    xpath:(//*[@class="sfx_buttonSvg_1250"])[2]
    sleep    3
    click on the button    Add columns
    click on the button    Other
	Sleep     2s
	Selenium2Library.Drag And Drop By Offset      xpath:(//*[@class="sfx_handle-holder_1167"])[6]      0      350
	Sleep      2s  
    ...
    sleep    5   
    Wait Until Page Contains Element      xpath:(//div[@class="flex-item flex-align-start sfx_DataCanvasToolbarButtonIcon_1341"])[5]    timeout=15
    Click Element    xpath:(//div[@class="flex-item flex-align-start sfx_DataCanvasToolbarButtonIcon_1341"])[5]
	wait for page to load
	capture page screenshot
...
Select KPIs
    ...
    FOR    ${kpi}    IN    @{list}
		    ...
		    IF    "${status}"=="False"
		        FOR    ${i}    IN RANGE    1    15
			        sleep    10
				    wait until element is visible    xpath: ((//div[@class="valueCellsContainer"])[3])//div[@name="valueCellCanvas"]    30
				    ${text}=    selenium2library.get text    xpath: ((//div[@class="valueCellsContainer"])[3])//div[@name="valueCellCanvas"]
				    ${status}=    run keyword and return status    should contain    ${text}    ${kpi}	    ignore_case=true
...
Verify that the results are displayed for selected filter values
	...
	Wait Until Page Contains Element      xpath://div[@class='cell-text' and text()='${FilterValue}']    timeout=20  																
...
Enter the Calendar Interval
	...
	#Clear Element Text      xpath:(//input[@class="sf-element sf-element-control sfc-property sfc-text-box"])[6]
	sleep    3
	...
	#Selenium2Library.Input Text     xpath:(//input[@class="sf-element sf-element-control sfc-property sfc-text-box"])[6]    ${startDate}
	sleep    2
	click on scroll up button    7    12
	click on scroll down button    7    12
	#Clear Element Text      xpath:(//input[@class="sf-element sf-element-control sfc-property sfc-text-box"])[9]
	sleep    2
	...
	click on scroll up button    7    12
	click on scroll down button    7    12
	#Selenium2Library.Input Text     xpath:(//input[@class="sf-element sf-element-control sfc-property sfc-text-box"])[9]    ${endDate}
	capture page screenshot
