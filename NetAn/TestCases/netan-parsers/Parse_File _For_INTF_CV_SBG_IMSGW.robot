...
Parse file for INTF_CV_SBG_IMSGW
    execute the command     cd /eniq/data/pmdata/eniq_oss_1/InformationStore/dc_cv_imsgw_sbg/
...
    place the file in the directory    ${date}.txt    ${EXEC_DIR}\\ENIQ_TC_Automation\\NetAn\\Other\\Files    /eniq/data/pmdata/eniq_oss_1/InformationStore/dc_cv_imsgw_sbg/
	...
	select the package    INTF_CV_SBG_IMSGW-eniq_oss_1
	click on button    ETLC Set History
	click on the button    Search
	click on link	Adapter_INTF_CV_SBG_IMSGW_information_store_parser
	sleep    20
	Verify the parser logs for SBG_IMSGW        
	${db_result}=    Query Sybase database    select * from DC_CV_IMSGW_SBG_RAW
