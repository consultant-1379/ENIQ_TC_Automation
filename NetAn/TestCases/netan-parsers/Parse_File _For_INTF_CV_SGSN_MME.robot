...
Parse file for INTF_CV_SGSN_MME
...
    place the file in the directory    ${date}.txt    ${EXEC_DIR}\\ENIQ_TC_Automation\\NetAn\\Other\\Files    /eniq/data/pmdata/eniq_oss_1/InformationStore/dc_cv_sgsn_mme/
	...
	select the package    INTF_CV_SGSN_MME-eniq_oss_1
	click on button    ETLC Set History
	click on the button    Search
	click on link	Adapter_INTF_CV_SGSN_MME_information_store_parser
	sleep    20
	Verify the parser logs    dc_cv_sgsn_mme    INTF_CV_SGSN_MME-eniq_oss_1
	${db_result}=    Query Sybase database    select * from DC_CV_SGSN_MME_RAW
    Verify that the table has data for this table    ${db_result}    DC_CV_SGSN_MME_RAW
