*** Settings ***
Documentation     Report Creation for multiple measure type for specific index
...
Verify Report creation for multiple measure type combination for specific index
    ...
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue on Failure    Report Creation with different measure type combination for specific index        ${object}[${key}]	    
    END



*** keywords ***
Report Creation with different measure type combination for specific index
    [Arguments]      ${data}   
    ...
    Select KPIs    ${data}[Measure_2]
    Set FlexFilterValues to     Custom
    Fetch Flex filter values
	${present}     @{list_value}=     Select Flex filter value as    ${data}[Index]
	IF         ${present}!= 0 
		Click on button value to add flex filter   
    END		
    Select KPIs    ${data}[Measure_1]
	Select KPIs    ${data}[Measure_3]
    ...
    Switch report page    pm_DC_E_ERBS_EUTRANCELLFDD_RAW(Cell Level)__NetAn_ODBC
