*** Settings ***
Documentation     Verify report creation for'Existing Interval Definition', 'Collection' and 'Collection'
...
*** Test Cases *** 
Verify Report creation in PMEX for'Existing Interval Definition', 'Collection' and 'Collection'
    [Tags]     PMEX_CDB          Report_Manager    TC_2256
    ${json}=    OperatingSystem.get file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMEXWebUI_Existing_Interval_Definition_Collection_Collection.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
	${count}=       set variable     0
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue On Failure    Verify report creation for'Existing Interval Definition', 'Collection' and 'Collection'       ${object}[${key}]
    END    
    
*** keywords ***
Verify report creation for'Existing Interval Definition', 'Collection' and 'Collection'
    [Arguments]      ${data}
