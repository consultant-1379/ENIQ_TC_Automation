*** Settings ***
Documentation     Validate the ObjectAggregation with All selected & GetData For Nodes 
...
Verify Report creation in PMEX and Validate the ObjectAggregation with All selected & GetData For Nodes
    [Tags]     PMEX          Report_Manager
    ${json}=    OperatingSystem.get file    ./ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMEXWebUI_Preceding_Period_All_Selected_Nodes.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue On Failure    Validate the ObjectAggregation with All selected & GetData For Nodes     ${object}[${key}] 
    END  


*** keywords ***

Validate the ObjectAggregation with All selected & GetData For Nodes 
    [Arguments]     ${data}
	...
	Verify saved report available in Report manager GUI     ${report_name}    Public   test report
