*** Settings ***
Documentation     Verify Report Creation for Preceding period no aggregation and Subnetwork 
...
Verify Report creation in PMEX  
    ...
    ${json}=    OperatingSystem.get file    ./ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMEXWebUI_Preceding_Period_No_Aggregation_Subnetwork.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue On Failure    Verify Report Creation for Preceding period no aggregation and Subnetwork    ${Object}[${Key}] 
    END  


*** keywords ***

Verify Report Creation for Preceding period no aggregation and Subnetwork 
    [Arguments]     ${data}
    ...
    ${report_name}=    Enter details to save report to library    report_name       Public       test
