*** Settings ***
Documentation     Verify Report Creation for Preceding period node and network 
...
Verify Report creation in PMEX  
    ...
    ${json}=    OperatingSystem.get file    ./ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMEXWebUI_Preceding_Period_Node_Network.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue On Failure    Verify Report Creation for Preceding period node and network    ${Object}[${Key}] 
    END  


*** keywords ***

Verify Report Creation for Preceding period node and network 
    [Arguments]     ${data}
