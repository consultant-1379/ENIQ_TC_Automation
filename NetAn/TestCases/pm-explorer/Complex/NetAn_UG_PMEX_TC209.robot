...
Verify Report creation in PMEX for 'Get data for' Dropdown 'Node(s)' and Object Aggregation for 'No Aggregation' with Calendar Interval
    [Tags]       PMEX_CDB     Report_Manager     NetAn_UG_PMEX_TC209
    ${json}=    OperatingSystem.get file    ./ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMExWebUIReport_CalendarInterval_NoAggregation_Nodes.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
	${count}=       set variable     0
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue on Failure    Report Creation with Input 'Get data for' Dropdown 'Node(s)' and Object Aggregation for 'No Aggregation' with Calendar Interval    ${object}[${key}]
