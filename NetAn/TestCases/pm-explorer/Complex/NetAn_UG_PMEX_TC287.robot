*** Settings ***
Documentation     Verify report creation for'Calendar Interval', 'Subnetwork' and 'Subnetwork'
...
Verify Report creation in PMEX with data 'Calendar Interval', 'Subnetwork' and 'Subnetwork'
    [Tags]       PMEX      Report_Manager
    ${json}=    OperatingSystem.get file    ./ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/PMExWebUIReport_CalendarInterval_Subnetwork_Subnetwork.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
	${count}=       set variable     0
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue on Failure    Verify report creation for'Calendar Interval', 'Subnetwork' and 'Subnetwork'    	${object}[${key}]
...
*** keywords ***
Verify report creation for'Calendar Interval', 'Subnetwork' and 'Subnetwork'
    [Arguments]      ${data}
