...
*** Test Cases ***
Verify Report creation in PMEX for cNELS Counters
    [Tags]     PMEX_CDB      EQEV-131255       NetAn_UG_PMEX_TC312          
    ${json}=    OperatingSystem.get file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/pm-explorer/cNELS_PMEx_DataIntegerity.json
    &{object}=    Evaluate     json.loads('''${json}''')     json
    FOR   ${key}   IN  @{object.keys()}
        Run Keyword And Continue On Failure   Verify report creation for cNELS Counters     ${object}[${key}]
    END    
    
*** keywords ***
Verify report creation for cNELS Counters
    [Arguments]      ${data}
