*** Settings ***
Documentation     Validate NR Historical Trend for 6M for Uplink UL Throughput >= 50.00 Mbps in NR App coverage
...
*** Test Cases ***
Validate NR Historical Trend for 6M for Uplink UL Throughput >= 50.00 Mbps
	[Tags]    NR_AC_KGB    NetAn_UG_NRAC_09        NR_AC_CDB
	...
	${DateValue} =    get NR date value
	Select NR name    G2RBS01
	${CellFailureRate}    ${avgFailedSessions}=	 	record the tooltip values for Selected cells
	${sql_query}=      get sql query from json file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/nr-app-coverage/nrappCoverageDataIntegrity.json    TC07
	...
	${CellFailureRate1}=	 	record the tooltip values for Parent nodes
	${sql_query1}=      get sql query from json file    ${EXEC_DIR}/ENIQ_TC_Automation/NetAn/Resources/Data/nr-app-coverage/nrappCoverageDataIntegrity.json    TC06
    ...
    Compare UI and DB values   ${CellFailureRate1}     ${DB_VALUE1}
	Capture page screenshot
