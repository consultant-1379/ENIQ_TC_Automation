...
verify snapshot removal on legacy servers
    ...
    IF    "${output}"=="Gen10 Plus"
        ${op1}=    Execute Command   lvs | grep -i homesnap
        Should Be Empty     ${op1}

        ${op2}=    Execute Command   lvs | grep -i rootsnap
        Should Be Empty     ${op2}

        ${op3}=    Execute Command   lvs | grep -i varlogsnap
        Should Be Empty     ${op3}

        ${op4}=    Execute Command   lvs | grep -i varsnap
        Should Be Empty     ${op4}

        ${op5}=    Execute Command   lvs | grep -i vartmpsnap
        Should Be Empty     ${op5}
    END