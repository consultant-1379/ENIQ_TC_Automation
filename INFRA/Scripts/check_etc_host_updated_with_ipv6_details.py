...
import os
import paramiko
def check_etc_host_updated_with_ipv6_details(hostname,user,pwd):
    ...
    for i in range(0,len(new_1)):
         ...
         if(ip_type=="IPv6"):
             ...
             etc_host_check=opensession_runcommand.connect_to_terminal_runcommand(host=hostname,username=user,password=pwd,command="cat /etc/hosts | grep "+new_1[i])
             if ip_addr not in etc_host_check:
                 flag=1
    if flag==0:
        return("True")
    else:
