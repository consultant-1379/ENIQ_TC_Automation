...
def check_mws_history_file_contain_data(hostname,user,pwd):
    mwshistoryfile =  opensession_runcommand.connect_to_terminal_runcommand(host=hostname,username=user,password=pwd,command="cat /eniq/installation/config/e | grep eniq_patch_history")
    count = 0
